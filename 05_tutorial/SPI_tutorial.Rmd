---
title: Tutorial for Statistical Performance Indicators
author: "Brian Stacy"
date: "`r Sys.Date()`"
...
This document contains a tutorial for the Statistical Performance Indicators (SPI).  The document contains a detailed description of each indicator, as well as the R code used to construct each indicator.  The R code can be modified to understand how changes to scoring can affect results.
...
spi_mapper('D3.1.AKI', 'SPI.D1.5.POV', 'Availability of Comparable Poverty Data' )
...
spi_mapper('D3.3.AKI', 'SPI.D1.5.CHLD.MORT', 'Data Use: Mortality rate, under-5 (per 1,000 live births)' )
...
spi_mapper('D3.15.AKI', 'SPI.D1.5.DT.TDS.DPPF.XP.ZS', 'Data Use: Debt service (PPG and IMF only, % of exports of goods, services and primary income)' )
...
```{r safelymanagedscored, exercise=TRUE}
...
spi_mapper('safe_managed_df', 'SPI.D1.5.SAFE.MAN.WATER', 'Data Use: Safely Managed Drinking Water' )
...
spi_mapper('lfp_data_df', 'SPI.D1.5.LFP', 'Data Use: Labor Force Participation' )
...
D2.1_DSDR <- df %>%
  ...
  select(iso3c,  date, RAW.D2.1.IDDS, SPI.D2.1.GDDS  ) %>%
  left_join(country_metadata)
...
```{r legislationgovernance, exercise=TRUE}
...
msc_df_map <- D5.2.1.SNAU %>%  filter(!is.na(SPI.D5.2.1.SNAU)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.2.NABY %>%  filter(!is.na(SPI.D5.2.2.NABY)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.3.CNIN %>%  filter(!is.na(SPI.D5.2.3.CNIN)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.4.CPIBY %>%  filter(!is.na(SPI.D5.2.4.CPIBY)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.5.HOUS %>%  filter(!is.na(SPI.D5.2.5.HOUS)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.6.EMPL %>%  filter(!is.na(SPI.D5.2.6.EMPL)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.7.CGOV %>%  filter(!is.na(SPI.D5.2.7.CGOV)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.8.FINA %>%  filter(!is.na(SPI.D5.2.8.FINA)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.9.MONY %>%  filter(!is.na(SPI.D5.2.9.MONY)) %>% left_join(country_metadata)
...
msc_df_map <- D5.2.10.GSBP %>%  filter(!is.na(SPI.D5.2.10.GSBP)) %>% left_join(country_metadata)
