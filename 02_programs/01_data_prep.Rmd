...
survey_fun  <- function(data, input_var) {
  ...
  for (i in 2004:2019) {
   temp <- cs_df %>%
    ...
    group_by(iso3c, country, date, input_var) %>% #group by country and create indicator for how many surveys over 10 year period
...
unesco_df <- unesco_df_raw %>%
    ...
    summarise(edu_admin_available=mean(as.numeric(edu_admin_available))) %>% #five year rolling average
    ungroup() %>%
    pivot_wider(
      names_from='type',
    ...
    group_by(iso3c) %>%
    arrange(-date, iso3c) %>%
    mutate(across(starts_with('RAW.D4.2.2.EDU'),~zoo::rollmean(.,k=5,fill=NA,align='left'),.names="{col}.MA5")) %>% #five year rolling average
    ungroup() %>%
...
ilo_df <- ilo_df %>%
  ...
  group_by(iso3c) %>%
  arrange(-date, iso3c) %>%
  mutate(across(starts_with('ILO.Source.type.filled'),~zoo::rollmean(.,k=5,fill=NA,align='left'),.names="{col}.MA5")) %>% #five year rolling average
  ungroup() %>%
...
#create an overall index score for research purposes
...
spi_index_df <- spi_index_df %>%
  select(-starts_with('SPI.D1'), -SPI.D5.3.DISK, -SPI.D4.3.GEO.second.admin.level,-SPI.D5.1.DILG,-SPI.D5.5.DIFI)



#get a list of oecd countries
oecd_country_query <-  GET(url = "https://api.worldbank.org/v2/country?region=OED&format=json") %>%
  content(Request_metadata, as = "text", encoding = "UTF-8") %>%
...
spi_index_df <- spi_index_df %>%
  ...
  mutate(across(c("SPI.D2.1.GDDS","SPI.D2.4.NADA","SPI.D4.1.1.POPU","SPI.D4.1.2.AGRI","SPI.D4.1.3.BIZZ","SPI.D4.1.4.HOUS","SPI.D4.1.5.AGSVY",
                  "SPI.D4.1.6.LABR","SPI.D4.1.7.HLTH","SPI.D4.1.8.BZSVY", "SPI.D4.2.1.SPL",
                  "SPI.D5.2.1.SNAU", "SPI.D5.2.2.NABY", "SPI.D4.2.4.LBR", "SPI.D5.2.1.SNAU","SPI.D5.2.2.NABY",                
                ...
                ~if_else((is.na(.) & (country %in% c(oecd_country_df$name,'Singapore'))),1,.)                )) %>% #fill in the missing values for OECDs with 1s because by participating in OECD these countries have this
...
spi_mapper('spi_index_df','SPI.INDEX','Overall SPI using Simple Index Averaging Over All Indicators.  Equal Weight for Each Indicator.  Fill Forward/Ignore Missing Values')
...
DT::datatable(index_disp, caption='Overall SPI using Simple Index Averaging Over All Indicators.  Equal Weight for Each Indicator.  Fill Forward/Ignore Missing Values',
                      rownames=TRUE,
...
### Scatterplot SCI SPI
spi_sci_plot <- ggplot(sci_spi_disp, aes(x=SPI.INDEX,y=SCI, color=region, alpha=date)) +
  # geom_point() +
...
spi_odin_plot <- ggplot(sci_spi_disp, aes(x=SPI.INDEX,y=ODIN_score, color=region, alpha=date)) +
  # geom_point() +
  geom_text(aes(label=iso3c)) +
  theme_bw() +
  labs(
...
cor(sci_spi_disp$SPI.INDEX,sci_spi_disp$SCI, use='pairwise.complete.obs')
cor(sci_spi_disp$SPI.INDEX,sci_spi_disp$ODIN_score, use='pairwise.complete.obs')





DT::datatable(sci_spi_disp, caption='Overall SPI using Simple Index Averaging Over All Indicators.  Equal Weight for Each Indicator.  Fill Forward/Ignore Missing Values',
                      rownames=TRUE,
                      colnames = c("Country", "ISO3C", "Date", "Income Group","Region",
                                   "SPI Index Value",
                                   "Statistical Capacity Index",
                                   "Statistical Capacity Index Rank",
                                   "Open Data Watch Overall Score",
                                   "Open Data Watch Overall Rank"
                                   ),
                      class='cell-border stripe',
                      escape = FALSE,
                      extensions = c ('Buttons', 'FixedHeader'), options=list(
...
```{r faolex, eval=FALSE, include=FALSE}


#load(paste(dir,"/Data/documentInfoNew Agri.Rdata", sep=""))
documentInfoNew <- readxl::read_excel(paste(dir,"/01_raw_data/1.2_DUNE/Faolex_inventory.xlsx", sep=""),
                                      col_types = c('text','text','text','text','text','text','text','text','date','date','text')) %>%
  as_tibble(.name_repair = 'universal') %>%
    mutate(Language.of.document=detect_language(Title, lang_code=FALSE), #check language      
           Language.of.document=str_to_lower(Language.of.document),
         Type.of.text=str_to_lower(Type.of.text),
         Main.area=str_to_lower(Primary.subject),
         Type.of.text=str_replace_all(Type.of.text,'[[:punct:]]',''),
         Main.area=str_replace_all(Main.area,'[[:punct:]]',''),
         url=Rel.file) 

#Now fix up the languages
documentInfoNew <- documentInfoNew %>%
  group_by(Country) %>%
  mutate(Language.of.document.typ=first(Language.of.document),
         Language.of.document=if_else(is.na(Language.of.document),Language.of.document.typ,Language.of.document), #replace language with language of another document in same country if missing
         Language.of.document=if_else(Language.of.document=='galican','spanish',Language.of.document), # correct one issue where galican was a recommended language
         Language.of.document=if_else(is.na(Language.of.document),'english',Language.of.document)) #as last resort, assume english document
...
ccodes <- read_csv(file=paste(dir, "/01_raw_data/1.2_DUNE/FAOSTAT_data_5-1-2020_country_codes.csv", sep="")) %>%
  as_tibble(.name_repair='universal') %>%
...
documentInfo <- documentInfo %>%
  mutate(Language.of.document=str_to_lower(Language.of.document),
         ...
         Type.of.text=str_replace_all(Type.of.text,'[[:punct:]]',''),
         Main.area=str_replace_all(Main.area,'[[:punct:]]',''),
         date=year(Date.of.original.text))

#keep just national policy and poverty reduction documents after year 2000
documentInfo <- documentInfo %>%
  filter(grepl("national development", Main.area)) %>%
  filter(Type.of.text=='policy') %>%
  filter(date>=2010)
...
```{r faolex_map, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
