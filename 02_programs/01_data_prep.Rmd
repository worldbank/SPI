  ...
  map_df <- get(data) %>%
   ...
   p1 <- ggplot() +
    ...
    labs(
      ...
      subtitle= 'Data Point is for last year available (typically 2019)',
...
The idea for this indicator is that countries should be producing statistical products that are utilized by academia.  As a measure of this, we use the IHSN database on censuses and surveys conducted by counties and calculate a ranking of countries based on the total number of downloads of the censuses and surveys produced by that country on a per capita basis.  In other words, we calculate the total number of downloads of censuses and surveys for a country, divided by the population of that country, to produce a ranking.
...
#do the plots  
   ...
   filter(academic_use_map, date==2019) %>%
    group_by(region) %>%
    filter(region!='Aggregates') %>%
    mutate(downloads_per_capita=mean(downloads_per_capita),
           Label = paste(round(mean(downloads_per_capita)), sep="")) %>%
  ...
  academic_use_map  %>%
    ...
    mutate(downloads_per_capita=if_else(is.na(downloads_per_capita), 0, downloads_per_capita)) %>%
    mutate(downloads_per_capita=mean(downloads_per_capita),
           Label = paste(mean(downloads_per_capita),'/',n(), sep="")) %>%
    ungroup() %>%
