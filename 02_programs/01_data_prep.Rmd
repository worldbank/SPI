...
load(paste(raw_dir,"/1.2_DUNE/documentInfoNew Agri.Rdata", sep=""))

#rename dataframe with document info
documentInfo <- documentInfoNew

#merge with WB compatible country codes
ccodes <- read_csv(file=paste(raw_dir,"/1.2_DUNE/FAOSTAT_data_5-1-2020_country_codes.csv", sep="")) %>%
...
```{r faolex_map, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r api_plots, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r twitter_map, echo=FALSE, message=FALSE, warning=FALSE}
...
openness_df_temp1 <- openness_df %>% 
  rename_with(~paste('RAW',., sep="."), .cols=c('Machine.readable', 'Non.proprietary', 'Download.options', 'Metadata.available', 'Terms.of.use', 'Openness.subscore') ) 
...
openness_df_temp2 <- openness_df %>% 
  ...
  rename_with(~paste('SPI',., sep="."), .cols=c('Machine.readable', 'Non.proprietary', 'Download.options', 'Metadata.available', 'Terms.of.use', 'Openness.subscore') ) 
...
#now do the figures 
cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.1.POPU.CENSUS))
spi_mapper('cs_df_map', 'RAW.D4.1.1.POPU.CENSUS', 'Population Census Available in Past 20 Years in 2019' )

cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.2.AGRI.CENSUS))
spi_mapper('cs_df_map', 'RAW.D4.1.2.AGRI.CENSUS', 'Agriculture Census Available in Past 20 Years in 2019' )

cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.3.BIZZ.CENSUS))
spi_mapper('cs_df_map', 'RAW.D4.1.3.BIZZ.CENSUS', 'Business/Establishment Census Available in Past 20 Years in 2019' )

cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.4.HOUS.SURVEYS))
spi_mapper('cs_df_map', 'RAW.D4.1.4.HOUS.SURVEYS', 'Household Survey Available in Past 20 Years in 2019' )

cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.5.AGRI.SURVEYS))
spi_mapper('cs_df_map', 'RAW.D4.1.5.AGRI.SURVEYS', 'Agriculture Survey Available in Past 20 Years in 2019' )

cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.6.LABR.SURVEYS))
spi_mapper('cs_df_map', 'RAW.D4.1.6.LABR.SURVEYS', 'Labor Force Survey Available in Past 20 Years in 2019' )

cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.7.HLTH.SURVEYS))
spi_mapper('cs_df_map', 'RAW.D4.1.7.HLTH.SURVEYS', 'Health Survey Available in Past 20 Years in 2019' )

cs_df_map <- cs_df %>%  filter(!is.na(RAW.D4.1.8.BIZZ.SURVEYS))
spi_mapper('cs_df_map', 'RAW.D4.1.8.BIZZ.SURVEYS', 'Business Survey Available in Past 20 Years in 2019' )
...
aspire_df <- aspire_df %>%
  group_by(iso3c) %>%
  mutate(ben_info_filled=if_else(ben_info==TRUE,as.numeric(ben_info), as.numeric(NA))) %>%
  fill(ben_info_filled, .direction='down') %>%
  mutate(SPI.D4.2.1.SPL=if_else(is.na(ben_info_filled),0, ben_info_filled),
         RAW.D4.2.1.SPL=ben_info) %>%
...
aspire_map <- aspire_df %>%
  filter(RAW.D4.2.1.SPL==TRUE)

spi_mapper('aspire_map', 'RAW.D4.2.1.SPL', 'Administrative Data Available on Beneficiary Counts or Expenditures using ASPIRE' )
...
unesco_df <- unesco_df_raw %>%
    ...
    left_join(country_metadata) %>%
    mutate(edu_admin_available=(!is.na(Value))
           ) %>%
    mutate(SPI.D4.2.2.EDU=as.numeric(edu_admin_available),
           RAW.D4.2.2.EDU=edu_admin_available) %>%
  select(c('iso3c', 'country', 'date', 'RAW.D4.2.2.EDU', 'SPI.D4.2.2.EDU')  )
...
unesco_map <- unesco_df %>%
  filter(RAW.D4.2.2.EDU==TRUE)
  

spi_mapper('unesco_map', 'RAW.D4.2.2.EDU', 'Education Administrative Data Available According to UIS' )
...
crvs_df <- jsonlite::fromJSON(Response_metadata, flatten = TRUE) %>%
  ...
  mutate(SPI.D4.2.3.CRVS=crvs,
         RAW.D4.2.3.CRVS=crvs) %>%
  left_join(spi_df_empty) #add on country metadata
...
crvs_map <- crvs_df %>%
  filter(!is.na(RAW.D4.2.3.CRVS))

spi_mapper('crvs_map', 'RAW.D4.2.3.CRVS', 'Complete Civil Registration & Vital Statistics Data Available According to WDI' )
...
geo_df <- geo_df %>%
    ...
    group_by( iso3c) %>%
    right_join(country_metadata) %>%
    mutate(RAW.D4.3.GEO.first.admin.level=if_else(is.na(First.administrative.level), 0, First.administrative.level),
           RAW.D4.3.GEO.second.admin.level=if_else(is.na(Second.administrative.level), 0, Second.administrative.level))    
  

#do some quick renaming and formatting
geo_df_temp1 <- geo_df %>% 
  rename_with(~paste('RAW',., sep="."), .cols=c('RAW.D4.3.GEO.first.admin.level', 'RAW.D4.3.GEO.second.admin.level') ) 


geo_df_temp2 <- geo_df %>% 
  mutate(across(c('RAW.D4.3.GEO.first.admin.level', 'RAW.D4.3.GEO.second.admin.level'), ~./100)) %>%
  rename_with(~paste('SPI',., sep="."), .cols=c('RAW.D4.3.GEO.first.admin.level', 'RAW.D4.3.GEO.second.admin.level') ) 
...
#read in csv file.
D5.1_DILG <- read_csv(file = paste(raw_dir, "5.1_DILG","5.1_DILG.csv", sep="/" )) %>%
  as_tibble(.name_repair = 'universal') %>%
  transmute(
    country=Country,
    date=Year,
    SPI.D5.1.LEGGOV=Data.Value
  ) %>%
    mutate(country=case_when(
    country=="Bahamas" ~ "Bahamas, The",                  
    country=="Bolivia (Plurinational State of)"   ~  "Bolivia"   ,                  
  ...
  mutate(SPI.D5.1.LEGGOV=if_else(SPI.D5.1.LEGGOV=='no data',as.character(NA), SPI.D5.1.LEGGOV)) 


D5.1_DILG_map <- D5.1_DILG %>%
  filter(!is.na(SPI.D5.1.LEGGOV))

spi_mapper('D5.1_DILG_map', 'SPI.D5.1.LEGGOV', 'National statistical legislation complies with United Nations Fundamental Principles of Statistics. Source: Paris21')
...
D5.2.2.NABY <- read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.2.NABY.csv", sep="/" )) %>%
  ...
  RAW.D5.2.1.NABY=NABY)  %>%
  select(iso3c, country, date, RAW.D5.2.1.NABY, SPI.D5.2.2.NABY  ) %>%
  arrange(date, country)
...
spi_mapper('msc_df_map', 'RAW.D5.2.1.SNAU', 'System of National Accounts in use in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.2.NABY))
spi_mapper('msc_df_map', 'RAW.D5.2.2.NABY', 'National Accounts base year in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.3.CNIN))
spi_mapper('msc_df_map', 'RAW.D5.2.3.CNIN', 'Classification of national industry in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.4.CPIBY))
spi_mapper('msc_df_map', 'RAW.D5.2.4.CPIBY', 'CPI base year in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.5.HOUS))
spi_mapper('msc_df_map', 'RAW.D5.2.5.HOUS', 'Classification of household consumption in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.6.EMPL))
spi_mapper('msc_df_map', 'RAW.D5.2.6.EMPL', 'Classification of status of employment in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.7.CGOV))
spi_mapper('msc_df_map', 'RAW.D5.2.7.CGOV', 'Central government accounting status in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.8.FINA))
spi_mapper('msc_df_map', 'RAW.D5.2.8.FINA', 'Compilation of government finance statistics in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.9.MONY))
spi_mapper('msc_df_map', 'RAW.D5.2.9.MONY', 'Compilation of monetary and financial statistics in 2019' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(RAW.D5.2.10.GSBP))
spi_mapper('msc_df_map', 'RAW.D5.2.10.GSBP', 'Business process in 2019' )
...
# save final dataset
spi_df_final <- spi_df %>%
  select(colnames(country_metadata), starts_with("SPI"), starts_with("RAW"))
