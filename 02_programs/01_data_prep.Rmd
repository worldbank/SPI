...
### SPI 1.5.1: Poverty Comparability
...
#Read in data from UN Inter-agency Group for Child Mortality Estimation
# 
# D3.3.AKI.MORT.QUAL <- read_csv(file=paste(raw_dir, '3.1_DPSS/D3.3_child_mort_RAW_v01.csv', sep="/")) %>%
...
#   mutate(LOWER_BOUND=max(LOWER_BOUND, na.rm=T),
#          UPPER_BOUND=max(UPPER_BOUND, na.rm=T)) %>%
#   filter(!between(OBS_VALUE, LOWER_BOUND,UPPER_BOUND)) %>%  # Also keep only surveys that were within the 90% confidence interval produced by UN IGME
#   filter(OBS_STATUS!='Normal value') %>% #drop the modelled estimates, which trivially satisfy
# 
# #Now loop from 2016 and 2019, keeping just data inside last 5 years.
# for (reference_year in 2004:2019) {
#   temp <-D3.3.AKI.MORT.QUAL %>%
...
for (reference_year in 2004:2019) {
  temp <-wbstats::wb(country="countries_only", 
          ...
          ungroup() %>%
          select(iso3c, country, date,  contains('SPI.D1.5.DT.TDS.DPPF.XP.ZS')) 
...
for (i in c(2004:2019)) {
  ...
  temp<-get(paste('D3.15.AKI_',i, sep=""))
...
aspire_fun <- function(date_start, date_end) {
  temp <- aspire_df %>%
      ...
      group_by(iso3c, country) %>%
      mutate(ben_info=if_else(ben_info==TRUE,as.numeric(ben_info), 0)) %>%
      summarise(ben_info=round(mean(ben_info),1) #get an average of this across countries
              ) %>%
      mutate(SPI.D4.2.1.SPL=if_else(is.na(ben_info),0, ben_info),
