...
spi_df <- bind_rows(replicate(length(span), country_metadata, simplify = FALSE), .id='date') %>%
...
D2.2_DSDR <- read_csv(file = paste(raw_dir, '/2.1_DSDR/', "D2.1.GDDS.csv", sep="" )) %>%
  ...
  select(iso3c, country, date, IDDS, D2.1.GDDS  ) %>%
  arrange(date, country)


D2.2_DSDR_2019 <- D2.2_DSDR %>%
  filter(date==2019) %>%
  filter(D2.1.GDDS>0)

spi_mapper('D2.2_DSDR_2019', 'IDDS', 'SDDS, SDDS Plus, or e-GDDS Data Standards in Place in 2019' )


#add to spi databases
spi_scores_df <- spi_scores_df %>%
  left_join(D2.2_DSDR)
...
openness_df <- bind_rows(openness_df_2016, openness_df_2017, openness_df_2018)
...
spi_mapper('cs_df_2019', 'POPU.CENSUS', 'Population Census Available in Past 20 Years in 2019' )
...
geo_df <- geo_df %>%
   ...
   geo_df %>%
    ...
    mutate(Percentage=100*mean(First.administrative.level),
           Label = paste(round(100*mean(First.administrative.level)), sep="")) %>%
    ggplot(aes(x=Percentage, y=region, fill=region)) +
      geom_bar(stat="identity",position='dodge') +
      geom_text(aes(label=Label)) +
      labs(
   ...
   ggplot() +
    ...
    mutate(Percentage=100*mean(Second.administrative.level),
           Label = paste(round(100*mean(Second.administrative.level)), sep="")) %>%
    ggplot(aes(x=Percentage, y=region, fill=region)) +
      geom_bar(stat="identity",position='dodge') +
      geom_text(aes(label=Label)) +
      labs(
