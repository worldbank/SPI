...
Find the following note on estimates:
...
1. Estimates are extended forwards four years from the latest estimate, and backwards four years from the earliest estimate

2. Estimates are extended forwards or backwards indefinitely, if the estimate is 99.5% or greater, or 0.5% or less. 
...
```{r safely_managed_scored, include=FALSE}
...
window_fun <- function(date_start, date_end) {
  ...
  mutate(data_quality=if_else((max_value>=99.5),1,as.numeric(data_quality))) %>% #because JMP imputes values indefinitely forward if value for water reaches >99.5%
...
Scoring:    
1 Point. Country has a labor force survey based estimate in past 5 years of labor force participation broken down by total, male, and female & estimated value from ILO is within 5 percentage points of value reported by national government.   
0.5 Point. Country has labor force survey or is within 5 points of ILO, but not both    
0 Points. Otherwise
...
ilo_fun <- function(date_start, date_end) {
  temp <- lfp_data %>%
    ...
    group_by(iso3c) %>%
    summarise(SPI.D1.5.LFP.ACC=as.numeric(mean(estimate_diff, na.rm=T)<=5),
