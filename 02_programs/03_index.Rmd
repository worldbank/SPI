...
The Index will contain only a subset of the sub-dimensions from our framework.  For dimensions excluded, we either lacked a source with a developed methodology or else the data collection for that measure was incomplete. This is described below:
  ...
  * **Indicator 1.5: Data use by international organizations:**  *Not included because of lack of established methodology*    
...
This process of eliminating some sub-dimensions due to lack of established methodology or country coverage results in 11 sub-dimensions.  This includes 3 on data services, 4 on data products, 3 on data sources, and 1 on data infrastructure.

After calculating the scores for each sub-dimension.  The SPI index is the average across the 11 sub-dimensions.  The SPI index has a maximum score of 100 and a minimum of 0.  A score of 100 would indicate that a country has every single element that we measure in place.  A score of 0 indicates that none are in place.  To be precise:

$$ SPI.INDEX_{ct} = \sum_{i=1}^N \frac{SPI.SUB.DIM_{ict}}{N} $$

Where SPI.INDEX is the SPI overall index.  SPI.SUB.DIM are the 11 SPI sub-dimensions listed above.  In the notation, c is a country, t is the date, i is a sub-dimension.
...
We compare the SPI index to the older World Bank Statistical Capacity Indicators (http://datatopics.worldbank.org/statisticalcapacity/SCIdashboard.aspx).  The correlation between the SPI index and the ODIN index is `r cor(comparison_df$SPI.INDEX,comparison_df$SCI, use='pairwise.complete.obs')`
...
We compare the SPI index to the Open Data Watch rankings of country statistical systems (https://odin.opendatawatch.com/report/rankings).  The correlation between the SPI index and the ODIN index is `r cor(comparison_df$SPI.INDEX,comparison_df$ODIN_score, use='pairwise.complete.obs')`
 `


```{r odin, include=FALSE}
