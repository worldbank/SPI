...
The time series for the dates prior to 2023 are identical as in the SPI release in February 2024.  Any revisions will only take place in consultation with the SPI advisory group.  This point is discussed further in SPI document [“Protocol for Updating the Statistical Performance Indicators (SPI)”](https://documents.worldbank.org/en/publication/documents-reports/documentdetail/099046201172417098/idu13723a9001b06e14d16184c015a331e52f40b).
...
```{r}
#county how many indicators showed improvement since 2016
    tile_df <- spi_agg_df %>%
      relocate(SPI.D3.13.CLMT, .after = SPI.D3.12.CNSP) %>%
      filter(between(date,start_date,end_date)) %>%
      select(date, c('SPI.D1.5.POV':'SPI.D5.5.DIFI')) %>%
      pivot_longer(
        cols=c('SPI.D1.5.POV':'SPI.D5.5.DIFI'),
        names_to = 'source_id',
        values_to = 'Score'
      ) %>%
  group_by(source_id) %>%
  summarise(Change=mean(Score[date==end_date], na.rm=TRUE)-mean(Score[date==start_date], na.rm=TRUE)) %>%
        left_join(metadata) %>%
  mutate(pos_change=Change>0)
...
reg_avg <- spi_index_df %>%
  filter(date==start_date | date==2020 | date==end_date) %>%
  group_by(date, region) %>%
  summarise(SPI.INDEX=mean(SPI.INDEX, na.rm=TRUE)) %>%
  mutate(date=factor(date, levels=c(start_date, 2020,end_date)),
         region=factor(region, levels=c("North America", "Europe & Central Asia","South Asia", "Latin America & Caribbean", "East Asia & Pacific", "Middle East & North Africa", "Sub-Saharan Africa")))

ggplot(reg_avg, aes(x=region,y=SPI.INDEX, fill=date,group = region,label=round(SPI.INDEX,0))) +
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge2(width = 1), size=5) +
...
ggplot(reg_avg, aes(x=region,y=SPI.INDEX, fill=date,group = region,label=round(SPI.INDEX,0))) +
  ...
  geom_text(position = position_dodge2(width = 1), size=5) +
  ylab("SPI Overall Score") +
  ggtitle(str_wrap('Unweighted Regional Average of SPI Overall Score Omitting Countries with Populations Smaller than Half a Million by Year',85)) +
  theme_minimal() +
  theme(legend.position='top',
        text = element_text(size = 14),
        axis.text.x=element_text(size = 14)) +
...
reg_fig_df <- reg_size_fig_df %>%
  ...
  group_by(region) %>%
  mutate(
    small_share=100*round(n/sum(n),3)
  )

  ggplot(reg_fig_df,  aes(x=region, y=SPI.INDEX, fill=small_pop,label=round(SPI.INDEX,0))) +
    geom_col(position = 'dodge2') +
    geom_text(position = position_dodge2(width = 1), size=4) +
    ylab("SPI Overall Score") +
    ggtitle('Average SPI Overall Score by Region and Population Size') +
    labs(fill="Population") +
    theme_minimal() +
    theme(legend.position='top',
          text = element_text(size = 14),
