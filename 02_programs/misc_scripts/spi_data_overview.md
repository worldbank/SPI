...
#do the plots  
   ggplot() +
    geom_map(data = filter(openness_df, date==end_date), aes(map_id = iso3c, fill = RAW.D2.2.Openness.subscore), map = maps$countries) + 
   ...
   filter(openness_df, date==end_date) %>%
    group_by(region) %>%
...
## Methodology
  ...
  2. Transform the data so that for each indicator we can create a score documenting whether a value exists for the country in a year, whether the value is based on country data, country data adjusted, estimated, or modelled data according the UN Stats metadata. **This will only include tier 1 indicators, as of November 2020.**.    
...
un_sdg_data_read  <- function(year) {
  read_csv(paste(raw_dir,'/3_DP/',year,'/SPI_D3_UNSD_data_5yr.csv',sep="")) %>%
...
#read in versions
#apply function from 2004 to 2023 and bind together
un_sdg_df <- bind_rows(lapply(2004:2023, un_sdg_data_read)) 
...
census_fun  <- function(data, input_var) {
  ...
  #read in csv file.
  cs_df <- read_csv(file = paste(raw_dir, '/4.1_SOCS/raw/', data, ".2023.csv", sep="" ))  %>% #bring in data collected for  2020 data
    ...
    select('iso3c','indicator_date','input_var')
  
    #append nada data on surveys
  cs_df <- cs_df %>%
    bind_rows(read_csv(file = paste(raw_dir, '/4.1_SOCS/', data, "_NADA.csv", sep="" )) %>% left_join(country_metadata)) %>%
    group_by(iso3c, indicator_date) %>%
    ...
    #save this intermediate dataset as csv
    write_excel_csv(cs_df, path = paste(raw_dir, '/4.1_SOCS/', data, "_annual.csv", sep="" ))
    
  
  #Now calculate our SPI score for this indicator
  for (i in 2004:end_date) {
   temp <- cs_df %>%
    mutate(date=i) %>%
    ...
    mutate(indicator_date=if_else(recency_indicator==TRUE, indicator_date, as.numeric(NA)),
         register=if_else(str_detect(input_var,"Register based census"),1,0)) %>%
    group_by(iso3c, date) %>%
    summarise(last_val=max(indicator_date, na.rm=T),
            register=max(register, na.rm=T),
            input_var=paste(unique(indicator_date), collapse=","),
            nada_dates=first(nada_dates)) %>%
    mutate(indicator=case_when(
    (date>=2021 & register==1) ~ 1, #score register based censuses
    (date-last_val<=10) & (date-last_val>=0) ~ 1,
    (date-last_val<=20) & (date-last_val>=0) ~ 0.5,
    TRUE ~ 0 )
    )  %>%
    ungroup() %>%
    select(c( 'iso3c', 'date', 'input_var','nada_dates', 'indicator')  ) %>%
    arrange(date, iso3c)
...
survey_fun  <- function(data, input_var) {
  ...
  cs_df <- read_csv(file = paste(raw_dir, '/4.1_SOCS/raw/', data, ".2023.csv", sep="" )) %>%        #add in 2020 manual data    
    write_excel_csv(path = paste(raw_dir, '/4.1_SOCS/', data, "_manual.csv", sep="" )) %>%
    distinct() %>%
    group_by( iso3c) %>% 
    rename(input_var = !! input_var) %>%
  ...
  cs_df <- cs_df %>%
    bind_rows(read_csv(file = paste(raw_dir, '/4.1_SOCS/', data, "_NADA.csv", sep="" ),
                       col_types = list(col_character(),col_double(), col_character())) %>% left_join(country_metadata)) %>%
    group_by(iso3c, indicator_date) %>%
    summarise(input_var=first(na.omit(input_var)  ),
            nada_dates=first(na.omit(nada_dates)  ) 
             ) %>%
  ...
  for (i in 2004:end_date) {
   temp <- cs_df %>%
    ...
    group_by( iso3c, date) %>% #group by country and create indicator for how many surveys over 10 year period
...
#Population Censuses
cs1_df <-census_fun('D4.1.1.CEN.POPU', 'POP.CENSUS') %>%
  rename(RAW.D4.1.1.POPU=input_var,
         SPI.D4.1.1.POPU=indicator) %>%
  dplyr::select(iso3c, date, starts_with("SPI"), starts_with("RAW"))

#Agriculture census  
cs2_df <-census_fun('D4.1.2.CEN.AGRI', 'AGRI.CENSUS') %>%
  rename(RAW.D4.1.2.AGRI=input_var,
...
cs4_df <-survey_fun('D4.1.4.SVY.HOUS', 'HOUS.SURVEYS') %>%
  ...
  dplyr::select(iso3c, date, starts_with("SPI"), starts_with("RAW")) %>%
  mutate(
    #adjust score for India. India has a poverty survey that is not produced by the national statistical office that is included in PIP.  Because it is produced by a private company that charges money for the survey, rather than produced by the NSO, it does not count
    SPI.D4.1.4.HOUS=if_else((iso3c=="IND" & date %in% c(2015:2023)),0,SPI.D4.1.4.HOUS),
    RAW.D4.1.4.HOUS.SURVEYS_nada=if_else((iso3c=="IND" & date %in% c(2015:2023)), "1977, 1983, 1987, 1993, 2004, 2009, 2011",RAW.D4.1.4.HOUS.SURVEYS_nada),
    RAW.D4.1.4.HOUS=if_else((iso3c=="IND" & date %in% c(2015:2023)), "1977, 1983, 1987, 1993, 2004, 2009, 2011",RAW.D4.1.4.HOUS)
  )
...
spi_mapper('cs_df_map', 'SPI.D4.1.1.POPU', 'Population Census Available in Past 20 Years in 2023' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.2.AGRI))
spi_mapper('cs_df_map', 'SPI.D4.1.2.AGRI', 'Agriculture Census Available in Past 20 Years in 2023' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.3.BIZZ))
spi_mapper('cs_df_map', 'SPI.D4.1.3.BIZZ', 'Business/Establishment Census Available in Past 20 Years in 2023' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.4.HOUS))
spi_mapper('cs_df_map', 'SPI.D4.1.4.HOUS', 'Household Survey Availablility in Past 10 Years in 2023' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.5.AGSVY))
spi_mapper('cs_df_map', 'SPI.D4.1.5.AGSVY', 'Agriculture Survey Availablility in Past 10 Years in 2023' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.6.LABR))
spi_mapper('cs_df_map', 'SPI.D4.1.6.LABR', 'Labor Force Survey Availablility in Past 10 Years in 2023' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.7.HLTH))
spi_mapper('cs_df_map', 'SPI.D4.1.7.HLTH', 'Health Survey Availablility in Past 10 Years in 2023' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.8.BZSVY))
spi_mapper('cs_df_map', 'SPI.D4.1.8.BZSVY', 'Business Survey Availablility in Past 10 Years in 2023' )
...
aspire_fun <- function(date_start, date_end) {
  temp <- aspire_df %>%
      ...
      select(c('iso3c',  'date', 'RAW.D4.2.1.SPL', 'SPI.D4.2.1.SPL')  )
...
####
#create this database for each year from 2004 to 2023 using a X year average
...
# 
# #read the data
# url <- 'https://api.uis.unesco.org/sdmx/data/UNESCO,SDG4,2.0/ADMIN.NB.._T._T._T.G2_3+GLAST............?startPeriod=2013&endPeriod=2020&format=csv-sdmx&locale=en&subscription-key=fd406abcbf3d4e318741b8537c92fe43'
...
unesco_df <- unesco_df_raw %>%
  ...
  select(c('iso3c',  'date', 'RAW.D4.2.2.EDU.NAS', 'SPI.D4.2.2.EDU')  )
...
df_birth <- read_csv(file=paste(raw_dir, '4.2_SOAD', 'SG_REG_BRTH90N.csv', sep="/")) %>% #Countries with birth registration data that are at least 90 percent complete (1 = YES; 0 = NO)
  filter(date>=2015 & date<=end_date) %>%
  ...
  #some countries do not update data ever year, so carry forward last value if missing
  group_by(iso3c) %>%
  mutate(across(starts_with("SPI"), na.locf, na.rm=FALSE)) %>% #use carry forward to most recent value
  mutate(across(starts_with("SPI"), ~if_else(is.na(.),0,.))) %>% #fill in missing values with 0
  ungroup() 
...
crvs_df <- spi_df_empty %>%
  ...
  ungroup() %>%
  select(c( 'iso3c', 'date','RAW.D4.2.3.BRTH90', 'RAW.D4.2.3.DETH75', 'RAW.D4.2.3.CRVS.WDI', 'SPI.D4.2.3.CRVS')  ) 
...
ilo_df <- ilo_df %>%
  ...
  dplyr::select(c('iso3c',  'date', 'SPI.D4.2.4.LBR'), starts_with('RAW.D4.2.4.LBR')  ) %>%
...
#bind different years together
geo_df <- bind_rows(geo_df_2015, geo_df_2016, geo_df_2017, geo_df_2018, geo_df_2020, geo_df_2021, geo_df_2022, geo_df_2023)

#for some small countries, the data at 1st admin level is not included.  These countries include:
# Andorra, Anguilla, Antigua and Barbuda, Bahrain, Dominica, Hong Kong, Macao, Maldives, Malta, Marshall Islands, 
# Micronesia, Monaco, Palau, Sao Tome and Principe, San Marino, Seychelles, Singapore, St. Kitts and Nevis, St. Lucia, 
# St. Vincent and the Grenadines, Tonga, and Liechtenstein

geo_country_list <- c('ADO','AIA','ATG','BHR','DMA', 'HKG','MAC','MDV', 'MLT', 'MHL',
                      'FSM', 'MCO', 'PLW','STP', 'SMR', 'SYC', 'SGP','KNA', 'LCA', 'VCT',
                      'TON', 'LIE')

geo_df <- geo_df %>%
  mutate(First.administrative.level=case_when(
      Country.Code %in% geo_country_list ~ Coverage.subscore,
      TRUE ~ First.administrative.level
          ),
    Second.administrative.level=case_when(
      Country.Code %in% geo_country_list ~ Coverage.subscore,
      TRUE ~ Second.administrative.level
          )
  )
...
#do the plots  
   ggplot() +
    geom_map(data = filter(geo_df, date==end_date), aes(map_id = iso3c, fill = RAW.D4.3.GEO.first.admin.level), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
  ...
  geo_df  %>%
   ...
   ggplot() +
    geom_map(data = filter(geo_df, date==end_date), aes(map_id = iso3c, fill = RAW.D4.3.GEO.second.admin.level), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
...
#read in previous vintage of SPI as starting point for previous years
vintage_year <- end_date-1
spi_previous_vintage_d5 <- read_csv(file=paste(raw_dir, "/previous_vintages/","SPI_data_",vintage_year,".csv", sep="" )) %>%
  select(iso3c, date, starts_with('RAW.D5'),starts_with('SPI.D5.')) %>%
  filter(date>=2016)
...
#read in csv file.
D5.2.3.CNIN <- read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.3.CNIN.2023.csv", sep="/" )) %>% #add in 2020 data
...
#read in csv file.
D5.2.4.CPIBY <- spi_previous_vintage_d5 %>%
...
D5.2.4.COICOP.WGT <-   read_excel(path=paste(raw_dir, "5.2_DISM","2024.02_Compiled Data for World Bank.xlsx", sep="/" ),
...
D5.2.4.CPI.REF <-  read_excel(path=paste(raw_dir, "5.2_DISM","2024.02_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet = "2. CPI Reference Period",
                              skip=2
                               ) %>%
  as_tibble(.name_repair='universal') %>%
...
D5.2.4.CPIBY.IMF.RAW <- D5.2.4.CPI.REF %>%
  filter(!is.na(iso3c)) %>%
  mutate(CPI_ref_period=str_sub(Common.Reference.Period,1,4),
  ...
  mutate(RAW.D5.2.4.CPIBY.IMF=if_else((CPI_weight_change>=2023 & CPI_ref_period<=2023), "annual chained",as.character(CPI_ref_period),as.character(CPI_ref_period))) %>%
...
#read in csv file.
D5.2.5.HOUS <- spi_previous_vintage_d5 %>%
...
D5.2.5.HOUS.IMF.2022 <-   read_excel(path=paste(raw_dir, "5.2_DISM","2022.10_Compiled Data for World Bank.xlsx", sep="/" ),
...
#read in data on COICOP from IMF
D5.2.5.HOUS.IMF.2023 <-   read_excel(path=paste(raw_dir, "5.2_DISM","2024.02_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet = "3. Household Consump Class",
                               skip=0
                               ) %>%
  as_tibble(.name_repair='universal') %>%  
  ...
  mutate(SPI.D5.2.5.HOUS.IMF=case_when(
    Status..as.of.January.2024=="COICOP" ~ 1, 
    Status..as.of.January.2024=="COICOP." ~ 1, 
    Status..as.of.January.2024=="COICOP 2018" ~ 1, 
    TRUE ~ 0 
  ),
  RAW.D5.2.5.HOUS.IMF=Status..as.of.January.2024) %>%
  mutate(IMF_code=as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  mutate(
        iso3c=if_else(iso3c=="KOS", 'XKX',iso3c) ) %>% #fix kosovo
...
D5.2.5.HOUS.IMF <- D5.2.5.HOUS.IMF.2022 %>% 
  bind_rows(D5.2.5.HOUS.IMF.2022 %>% mutate(date=2021)) %>%
  mutate(date=if_else(!is.na(date),2022,2021)) %>%
  bind_rows(D5.2.5.HOUS.IMF.2023 %>% mutate(date=2023)) %>%
  filter(!is.na(iso3c))
...
Classification of status of employment refers to employment data that are 
 ...
 of Status in Employment (ICSE-93).  It classifies jobs with respect to the type
...
D5.2.6.EMPL <- read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.6.EMPL.csv", sep="/" )) %>%
  ...
  bind_rows(read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.6.EMPL.2023.csv", sep="/" )) %>% mutate(date=2023, SPI.D5.2.6.EMPL=as.numeric(ISCE))) %>% # add in 2023 data  
  mutate(
...
#read in csv file.
D5.2.7.CGOV <- spi_previous_vintage_d5 %>%
...
# 2021
D5.2.7.CGOV.IMF.2021 <-   read_excel(path=paste(raw_dir, "5.2_DISM","2022.10_Compiled Data for World Bank.xlsx", sep="/" ),
  ...
  mutate(SPI.D5.2.7.CGOV.IMF=case_when(
    ...
    General.Government..consolidated.=="NA" ~ as.numeric(NA),
    TRUE ~ 0 
  ),
...
#2022
D5.2.7.CGOV.IMF.2022 <-   read_excel(path=paste(raw_dir, "5.2_DISM","2022.10_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet = "5. Cntrl Gct Acct Status",
                               skip=9
                               ) %>%
  as_tibble(.name_repair='universal') %>%  
...
D5.2.7.CGOV.IMF.2023 <-   read_excel(path=paste(raw_dir, "5.2_DISM","2024.02_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet = "5. Cntrl Gct Acct Status",
                               skip=9
                               ) %>%
  as_tibble(.name_repair='universal') %>%  
...
D5.2.7.CGOV.IMF <- bind_rows(
  D5.2.7.CGOV.IMF.2021,
  D5.2.7.CGOV.IMF.2022,
  D5.2.7.CGOV.IMF.2023
) %>%
  filter(!is.na(iso3c))
...
D5.2.7.CGOV <-  D5.2.7.CGOV %>%
  ...
  #use na.locf for some that are not updated by IMF
  arrange(date, iso3c) %>%
  group_by(iso3c) %>%
  mutate(
    SPI.D5.2.7.CGOV=na.locf(SPI.D5.2.7.CGOV, na.rm=FALSE)
  ) %>%
  ungroup() %>%
  filter(!is.na(SPI.D5.2.7.CGOV))
...
D5.2.8.FINA <- read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.8.FINA.csv", sep="/" )) %>%
  ...
  bind_rows(read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.8.FINA.2023.csv", sep="/" )) %>% mutate(date=2023, iso3c=Code)) %>% #add in 2020 data
...
#read in csv file.
D5.2.9.MONY <- spi_previous_vintage_d5 %>%
...
#read in data from IMF
D5.2.9.MONY.IMF.2022 <-   read_excel(path=paste(raw_dir, "5.2_DISM","2022.10_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet ="7. MFS Compilation status",
                               skip=0
                               ) %>%
  as_tibble(.name_repair='universal') %>%  
...
D5.2.9.MONY.IMF.2023 <-   read_excel(path=paste(raw_dir, "5.2_DISM","2024.02_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet ="7. MFS Compilation status",
                               skip=0
                               ) %>%
  as_tibble(.name_repair='universal') %>%  
  ...
  mutate(SPI.D5.2.9.MONY.IMF=case_when(
    grepl('MFSM 2000',Monetary.and.Financial.Statistics..MFS..Reporting.Status..as.of.January.2024) ~ 1, 
    TRUE ~ 0 
  ),
  RAW.D5.2.9.MONY.IMF=Monetary.and.Financial.Statistics..MFS..Reporting.Status..as.of.January.2024) %>%
  mutate(IMF_code=as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  select(iso3c, RAW.D5.2.9.MONY.IMF,SPI.D5.2.9.MONY.IMF ) 

D5.2.9.MONY.IMF <- D5.2.9.MONY.IMF %>%
  bind_rows(D5.2.9.MONY.IMF.2022 %>% mutate(date=2021)) %>%
  bind_rows(D5.2.9.MONY.IMF.2022 %>% mutate(date=2022)) %>%
  bind_rows(D5.2.9.MONY.IMF.2023 %>% mutate(date=2023))  %>%
  filter(!is.na(iso3c))
...
D5.2.10.GSBP <- read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.10.GSBP.csv", sep="/" )) %>%
  ...
  bind_rows(read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.10.GSBP.2023.csv", sep="/" ))%>% mutate(date=2023)) %>% #add in 2022 data
...
spi_mapper('msc_df_map', 'SPI.D5.2.1.SNAU', 'System of National Accounts in use in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.2.NABY))
spi_mapper('msc_df_map', 'SPI.D5.2.2.NABY', 'National Accounts base year in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.3.CNIN))
spi_mapper('msc_df_map', 'SPI.D5.2.3.CNIN', 'Classification of national industry in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.4.CPIBY))
spi_mapper('msc_df_map', 'SPI.D5.2.4.CPIBY', 'CPI base year in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.5.HOUS))
spi_mapper('msc_df_map', 'SPI.D5.2.5.HOUS', 'Classification of household consumption in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.6.EMPL))
spi_mapper('msc_df_map', 'SPI.D5.2.6.EMPL', 'Classification of status of employment in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.7.CGOV))
spi_mapper('msc_df_map', 'SPI.D5.2.7.CGOV', 'Central government accounting status in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.8.FINA))
spi_mapper('msc_df_map', 'SPI.D5.2.8.FINA', 'Compilation of government finance statistics in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.9.MONY))
spi_mapper('msc_df_map', 'SPI.D5.2.9.MONY', 'Compilation of monetary and financial statistics in 2020' )

msc_df_map <- D5.2.MSC %>%  filter(!is.na(SPI.D5.2.10.GSBP))
spi_mapper('msc_df_map', 'SPI.D5.2.10.GSBP', 'Business process in 2020' )
...
Two indicators from the GovTech Maturity Index are included in this dimension. These include the Core Government Systems Index (CGSI) and the GovTech Enablers Index (GTEI). From the documentation: https://www.worldbank.org/en/programs/govtech/gtmi
...
```{r}
#read in the GTMI data
gtmi_df <- read_excel(path=paste(raw_dir, "5.3_DISK", "WBG_GovTech Dataset_Oct2022.xlsx", sep="/"),
...
D5.3_DISK_map <- D5.3_DISK %>%
  filter(!is.na(D.5.3.CGSI))

spi_mapper('D5.3_DISK_map', 'D.5.3.CGSI', 'Core Government Systems Index. Source: World Bank')

spi_mapper('D5.3_DISK_map', 'D.5.3.GTEI', 'GovTech Enablers Index. Source: World Bank')
...
#pull from unsd api
D5.5_DIFI <- read_csv(file=paste(raw_dir, '5.5_DIFI', 'SG_STT_NSDSFND.csv', sep="/")) %>% 
  select(iso3c, date,ind_value) %>%
  right_join(spi_df_empty) %>%
...
spi_df_final <- spi_df_final %>%
  ...
  mutate(across(c(#"SPI.D1.5.POV",
                  ...
                  "SPI.D5.2.10.GSBP",
                  "SPI.D5.5.DIFI"),
                ~if_else(( (income_level=='High income')  & (country %in% country_mod_list)),1,.)                )) 
