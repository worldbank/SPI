...
# # pull all studiy citatinos from IHSN
# ihsn_citations_base_url <- "https://catalog.ihsn.org/index.php/api/citations" #define url
# study_request_ihsn_citations<-fromJSON(paste(ihsn_citations_base_url,"/search","?ps=10000", sep="")) # pull from url
# 
# study_citations_df_ihsn <- study_request_ihsn_citations$result$rows #convert to dataframe
# write_excel_csv(study_df_ihsn, path = paste(output_dir, "ihsn_library_public_studies.csv")) #write to csv
...
survey_df <- study_df_internal %>%
  ...
  summarise(freq=n(),
            title=first(title)) %>%
  group_by(country) %>%
  mutate(ihsn_dates = paste0(indicator_date, collapse = ", "),
         ihsn_title = paste0(title, collapse = "; ")) %>%
  select(country,indicator_date, ihsn_dates, ihsn_title)
...
saver('hh/ies|Income/Expenditure/Household Survey' , 'D4.1.4.SVY.HOUS_IHSN.csv')

saver('ag/oth|Agricultural Survey' , 'D4.1.5.SVY.AGRI_IHSN.csv')

saver('lfs' , 'D4.1.6.SVY.LABR_IHSN.csv')

saver('dhs|mics|whs|hea' , 'D4.1.7.SVY.HLTH_IHSN.csv')

saver('Business Survey' , 'D4.1.8.SVY.BIZZ_IHSN.csv')
...
survey_df <- study_df_internal %>%
  ...
  write_excel_csv(survey_df, path = paste(output_dir, 'D4.1.3.CEN.BIZZ_IHSN.csv', sep="")) #write to csv
