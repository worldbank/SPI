...
lfp_estimated <- read_csv(paste0(raw_dir,"/1.5_DUIO/ILO_LFP_modelled-2022-11-14.csv")) %>%
  as_tibble(.name_repair = 'universal') %>%
...
lfp_reported <- read_csv(paste0(raw_dir,"/1.5_DUIO/ILO_LFP_survey-2022-12-05.csv")) %>%
  as_tibble(.name_repair = 'universal') %>%
  mutate(country=case_when(
    ref_area.label=="Gambia" ~ "Gambia, The",
...
#   mutate(iso3c=if_else(is.na(Country.Code), Code, Country.Code),
#          country=Table.Name) %>%
#   select(c('iso3c',  'date',  'Latest.population.census')  ) %>%
...
#     )  %>%
#     ungroup() %>%
#     select(c('iso3c',  'date', 'Latest.population.census', 'indicator')  ) %>%
#     arrange(date, iso3c)
...
ag_census_df <- df %>%
  ...
  mutate(last_val=str_extract(Latest.agricultural.census, "\\d{4}") )%>%
    ...
    arrange(date, iso3c) %>%
  mutate(indicator=if_else((iso3c=="SGP" & date>=2015),1,indicator),
         Latest.agricultural.census=if_else((iso3c=="SGP" & date>=2015),'Admin data sources',Latest.agricultural.census)) #Singapore Stat provided the SPI team with documentation on their agriculture statistics. https://www.singstat.gov.sg/publications/reference/ebook/industry/agriculture-animal-production-and-fisheries
...
survey_fun  <- function(data, input_var) {
  ...
  cs_df <- read_csv(file = paste(raw_dir, '/4.1_SOCS/raw/', data, ".2022.csv", sep="" )) %>%        #add in 2020 manual data    
    write_excel_csv(path = paste(raw_dir, '/4.1_SOCS/', data, "_manual.csv", sep="" )) %>%
    distinct() %>%
    group_by( iso3c) %>% 
    rename(input_var = !! input_var) %>%
...
D5.2.4.COICOP.WGT <-   read_excel(path=paste(raw_dir, "5.2_DISM","2023.10_Compiled Data for World Bank.xlsx", sep="/" ),
...
D5.2.4.CPI.REF <-  read_excel(path=paste(raw_dir, "5.2_DISM","2023.10_Compiled Data for World Bank.xlsx", sep="/" ),
...
D5.2.4.CPIBY.IMF.RAW <- D5.2.4.CPI.REF %>%
  filter(!is.na(iso3c)) %>%
  mutate(CPI_ref_period=str_sub(CPI.reference.period..as.of.october.2023,1,4),
...
D5.2.5.HOUS.IMF <-   read_excel(path=paste(raw_dir, "5.2_DISM","2023.10_Compiled Data for World Bank.xlsx", sep="/" ),
  ...
  mutate(SPI.D5.2.5.HOUS.IMF=case_when(
    Status..as.of.October.2023=="COICOP" ~ 1, 
    Status..as.of.October.2023=="COICOP." ~ 1, 
    Status..as.of.October.2023=="COICOP 2018" ~ 1, 
    TRUE ~ 0 
  ),
  RAW.D5.2.5.HOUS.IMF=Status..as.of.October.2023) %>%
  mutate(IMF_code=as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  mutate(
        iso3c=if_else(iso3c=="KOS", 'XKX',iso3c) ) %>% #fix kosovo
...
D5.2.6.EMPL <- read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.6.EMPL.csv", sep="/" )) %>%
  ...
  bind_rows(read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.6.EMPL.2023.csv", sep="/" )) %>% mutate(date=2022, SPI.D5.2.6.EMPL=as.numeric(ISCE))) %>% # add in 2023 data  
  mutate(
...
D5.2.7.CGOV.IMF <-   read_excel(path=paste(raw_dir, "5.2_DISM","2023.10_Compiled Data for World Bank.xlsx", sep="/" ),
...
#read in data from IMF
D5.2.9.MONY.IMF.UPD <-   read_excel(path=paste(raw_dir, "5.2_DISM","2023.10_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet ="7. MFS Compilation status",
                               skip=0
                               ) %>%
  as_tibble(.name_repair='universal') %>%  
  ...
  mutate(SPI.D5.2.9.MONY.IMF=case_when(
    grepl('MFSM 2000',Monetary.and.Financial.Statistics..MFS..Reporting.Status..as.of.October.2023) ~ 1, 
    TRUE ~ 0 
  ),
  RAW.D5.2.9.MONY.IMF=Monetary.and.Financial.Statistics..MFS..Reporting.Status..as.of.October.2023) %>%
  mutate(IMF_code=as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  select(iso3c, RAW.D5.2.9.MONY.IMF,SPI.D5.2.9.MONY.IMF ) 
