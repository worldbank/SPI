...
D3.3.AKI.MORT <- read_csv(file=paste(raw_dir, '1.5_DUIO/UN IGME Child Mortality and Stillbirth Estimates_2022.csv', sep="/")) %>%
  ...
  left_join(read_csv(file = paste(raw_dir, "/metadata/WDI_metadata_",2020,".csv", sep="" )))
...
lfp_reported <- read_csv(paste0(raw_dir,"/1.5_DUIO/ILO_LFP_survey-2022-11-14.csv")) %>%
  as_tibble(.name_repair = 'universal') %>%
  mutate(country=case_when(
...
```{r online_access}
  ...
  #read in ODIN data
  for (i in c(2015,2016,2017,2018,2020, 2021)) {
...
#bind different years together
geo_df <- bind_rows(geo_df_2015, geo_df_2016, geo_df_2017, geo_df_2018, geo_df_2020, geo_df_2021)
...
D5.1_DILG <- un_pull('SG_STT_FPOS', '2004', '2022') %>%
...
D5.2.3.CNIN <- read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.3.CNIN.csv", sep="/" )) %>%
  bind_rows(read_csv(file = paste(raw_dir, "5.2_DISM","D5.2.3.CNIN.2020.csv", sep="/" ))) %>% #add in 2020 data
...
D5.2.4.CPIBY.IMF.RAW <- D5.2.4.CPI.REF %>%
  ...
  mutate(RAW.D5.2.4.CPIBY.IMF=if_else((CPI_weight_change>=2020 & CPI_ref_period<2022), "annual chained",as.character(CPI_ref_period))) %>%
