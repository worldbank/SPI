...
out_dir: "`r paste0(here::here(),'/04_reports')`"
---
...
out_dir <- paste(dir, '03_output_data', sep="/")
...
write_excel_csv(aspire_df, paste(out_dir, 'SPL_admin_data_availability.csv', sep="/"))
...
spi_df_final %>%
  mutate(across(is.list, as.character)) %>%
  write_excel_csv( path = paste(out_dir, 'SPI_data.csv', sep="/"))
...
spi_df_final %>%
  ...
  select(-lat,-long) %>%
  write_dta( path = paste(out_dir, 'SPI_data.dta', sep="/"))
