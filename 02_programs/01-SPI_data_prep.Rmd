...
for (reference_year in 2004:end_date) {
  ...
  temp_5<-df %>%
        ...
        dplyr::group_by(countrycode, comparability) %>% #for each country and comparability type, get number of comparable estimates
...
ag_census_df <- df %>%
  ...
  mutate(last_val=str_extract(Latest.agricultural.census, "\\d{4}") )%>%
    ...
    arrange(date, iso3c) %>%
  mutate(indiator=if_else((iso3c=="SGP" & date>=2015),1,indicator),
