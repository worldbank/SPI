...
```{r online_access}
#read in ODIN data
  for (i in span[-which(span == 2019 | span < 2015)]) {
...
```{r geo}
  ...
  #read in ODIN data
  #read in ODIN data
  for (i in c(2015,2016,2017,2018,2020, 2021, 2022, 2023, 2024)) {
...
#bind different years together
geo_df <- bind_rows(geo_df_2015, geo_df_2016, geo_df_2017, geo_df_2018, geo_df_2020, geo_df_2021, geo_df_2022, geo_df_2023, geo_df_2024)
...
D5.5_DIFI <- 
  ...
  select(iso3c, 
         ...
         ind_value) %>%
  right_join(spi_df_empty) %>%
  mutate(RAW.D5.5.DIFI = if_else((is.na(ind_value) | 
                                    ind_value == "NaN"),
                                 as.numeric(NA), 
                                 as.numeric(ind_value)),
         SPI.D5.5.DIFI = if_else((is.na(ind_value) | 
                                    ind_value == "NaN"),
                                 as.numeric(NA), 
                                 as.numeric(ind_value))) %>%
  select(-ind_value)
