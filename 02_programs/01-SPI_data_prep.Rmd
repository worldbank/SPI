...
topics_list <- unique(study_df_ihsn$topic)
  ...
  academic_use_df <- academic_use_df_2020
...
#do the plots  
   ggplot() +
    geom_map(data = filter(academic_use_map, date==2020), aes(map_id = iso3c, fill = academic_use_rank), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
   ...
   filter(academic_use_map, date==2020) %>%
    group_by(region) %>%
    filter(region!='Aggregates') %>%
...
for (reference_year in 2004:2020) {
  ...
  temp_5<-df %>%
        ...
        dplyr::select( country,iso3c, date, starts_with('SPI.D1.5.POV')) 
...
D3.3.AKI.MORT <- read_csv(file=paste(raw_dir, '1.5_DUIO/UN IGME Child Mortality and Stillbirth Estimates_2020.csv', sep="/")) %>%
...
for (reference_year in 2004:2020) {
  ...
  temp_5 <-D3.3.AKI.MORT %>%
          ...
          mutate(date=reference_year) %>%
          dplyr::select( country, date, starts_with('SPI.')) %>%
...
# 8 Year window
####
#create this database for each year from 2004 to 2020 using a 8 year window
...
lfp_estimated <- read_csv(paste0(raw_dir,"/1.5_DUIO/ILO_LFP_modelled-2021-06-25.csv")) %>%
  as_tibble(.name_repair = 'universal') %>%
  mutate(country=case_when(
...
lfp_reported <- read_csv(paste0(raw_dir,"/1.5_DUIO/ILO_LFP_survey-2021-06-25.csv")) %>%
  as_tibble(.name_repair = 'universal') %>%
  mutate(country=case_when(
    ref_area.label=="Gambia" ~ "Gambia, The",
    TRUE ~ ref_area.label
    ...
    left_join(ilo_ref_area) %>%
  select(iso3c, date, source.label, reported_value )
...
lfp_data <- lfp_reported %>%
  ...
  mutate(estimate_diff=if_else(!is.na(estimated_value),reported_value-estimated_value,0),
         lfs_source=(grepl("LFS",source.label))) %>%
...
openness_df <- bind_rows(openness_df_2015, openness_df_2016, openness_df_2017, openness_df_2018,openness_df_2020)
...
#do the plots  
   ggplot() +
    geom_map(data = filter(openness_df, date==2020), aes(map_id = iso3c, fill = RAW.D2.2.Openness.subscore), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
...
census_fun  <- function(data, input_var) {
  ...
  #read in csv file.
  cs_df <- read_csv(file = paste(raw_dir, '/4.1_SOCS/raw/', data, ".2020.csv", sep="" ))  %>% #bring in data collected for  2020 data
...
cs3_df <-census_fun('D4.1.3.CEN.BIZZ', 'BIZZ.CENSUS') %>%
  ...
  dplyr::select(country, date, starts_with("SPI"), starts_with("RAW"))
...
spi_mapper('cs_df_map', 'SPI.D4.1.1.POPU', 'Population Census Available in Past 20 Years in 2020' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.2.AGRI))
spi_mapper('cs_df_map', 'SPI.D4.1.2.AGRI', 'Agriculture Census Available in Past 20 Years in 2020' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.3.BIZZ))
spi_mapper('cs_df_map', 'SPI.D4.1.3.BIZZ', 'Business/Establishment Census Available in Past 20 Years in 2020' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.4.HOUS))
spi_mapper('cs_df_map', 'SPI.D4.1.4.HOUS', 'Household Survey Availablility in Past 10 Years in 2020' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.5.AGSVY))
spi_mapper('cs_df_map', 'SPI.D4.1.5.AGSVY', 'Agriculture Survey Availablility in Past 10 Years in 2020' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.6.LABR))
spi_mapper('cs_df_map', 'SPI.D4.1.6.LABR', 'Labor Force Survey Availablility in Past 10 Years in 2020' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.7.HLTH))
spi_mapper('cs_df_map', 'SPI.D4.1.7.HLTH', 'Health Survey Availablility in Past 10 Years in 2020' )

cs_df_map <- cs_df %>%  filter(!is.na(SPI.D4.1.8.BZSVY))
spi_mapper('cs_df_map', 'SPI.D4.1.8.BZSVY', 'Business Survey Availablility in Past 10 Years in 2020' )
...
####
#create this database for each year from 2004 to 2020 using a X year average
for (i in c(2004:2020)) {
...
ilo_df <- ilo_df %>%
  ...
  dplyr::select(c('iso3c', 'country', 'date', 'SPI.D4.2.4.LBR'), starts_with('RAW.D4.2.4.LBR')  ) %>%
...
#bind different years together
geo_df <- bind_rows(geo_df_2015, geo_df_2016, geo_df_2017, geo_df_2018, geo_df_2020)
...
#do the plots  
   ggplot() +
    geom_map(data = filter(geo_df, date==2020), aes(map_id = iso3c, fill = RAW.D4.3.GEO.first.admin.level), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
  ...
  geo_df  %>%
   ...
   ggplot() +
    geom_map(data = filter(geo_df, date==2020), aes(map_id = iso3c, fill = RAW.D4.3.GEO.second.admin.level), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
...
D5.1_DILG <- un_pull('SG_STT_FPOS', '2004', '2020') %>%
...
# can find some of this data from ILO at this link:
...
#pull from unsd api
D5.5_DIFI <- un_pull('SG_STT_NSDSFND', '2004', '2020') %>%
  select(iso3c, date,value) %>%
  right_join(spi_df_empty) %>%
...
#pull from unsd api
D5.5_DIFI_test <- un_pull('SG_STT_NSDSFDGVT', '2004', '2020') %>%
  select(iso3c, date,value) %>%
  right_join(spi_df_empty) %>%
  mutate(RAW.D5.5.DIFI=if_else((is.na(value) | value=="NaN"),as.numeric(NA), as.numeric(value)),
         SPI.D5.5.DIFI=if_else((is.na(value) | value=="NaN"),as.numeric(NA), as.numeric(value))) %>%
...
spi_df_final <- spi_df %>%
  dplyr::select(country, iso3c, date, starts_with("SPI"), starts_with("RAW")) %>%
  right_join(spi_df_empty) %>%
  left_join(pop_df) %>%
  arrange(-date,country)
...
####
#lining up data to 2020 and filling in some values for OECD countries.  
####
