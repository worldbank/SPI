...
AFG_df2 <- hyper_read('https://washdata.org/data/country/AFG/household/download')
...
un_sdg_df <- bind_rows(lapply(2004:2024, un_sdg_data_read)) 
...
```{r geo}
  ...
  #read in ODIN data
  #read in ODIN data
  for (i in c(2015,2016,2017,2018, 2019,2020, 2021, 2022, 2023, 2024)) {
...
#bind different years together
geo_df <- bind_rows(geo_df_2015, geo_df_2016, geo_df_2017, geo_df_2018, geo_df_2019, geo_df_2020, geo_df_2021, geo_df_2022, geo_df_2023, geo_df_2024)
...
imf_file <- "2025.04_Compiled Data for World Bank_V2.xlsx"
...
D5.2.5.HOUS.IMF.2024 <- read_excel(path  = paste(raw_dir, 
                                                 "5.2_DISM",
                                                 "2025.04_Compiled Data for World Bank_V2.xlsx", 
                                                 sep = "/" ),
                                   sheet = "3. Household Consump Class",
                                   skip  = 0) %>%
  as_tibble(.name_repair = 'universal') %>%  
  rename(IMF_code    = Code,
         IMF_country = Country) %>%
  mutate(SPI.D5.2.5.HOUS.IMF = case_when(Status..as.of.April.2025=="COICOP" ~ 1, 
                                         Status..as.of.April.2025=="COICOP." ~ 1, 
                                         Status..as.of.April.2025=="COICOP 2018" ~ 1, 
                                         TRUE ~ 0),
         RAW.D5.2.5.HOUS.IMF = Status..as.of.April.2025) %>%
  mutate(IMF_code            = as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  mutate(iso3c = if_else(iso3c == "KOS", 
                         'XKX',
...
D5.2.9.MONY.IMF.2024 <-  
  ...
  mutate(SPI.D5.2.9.MONY.IMF = case_when(
    grepl('MFSM 2000',
          Monetary.and.Financial.Statistics..MFS..Reporting.Status..as.of.April.2025) ~ 1, 
    TRUE ~ 0),
    RAW.D5.2.9.MONY.IMF = Monetary.and.Financial.Statistics..MFS..Reporting.Status..as.of.April.2025) %>%
  mutate(IMF_code = as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  select(iso3c, 
         RAW.D5.2.9.MONY.IMF,
