---
title: 'SPI Update: What''s Changed?'
author: "SPI Team"
date: "`r Sys.Date()`"
output:
  bookdown::word_document2: 
    fig_width: 9
    fig_height: 6
abstract: 'In March 2023, the second vintage of the SPI data was released.  This document provides an overview of the major changes to the scores compared to the intial data release in March 2021.  In this new edition, SPI scores for the calendar year 2020, 2021, and 2022 have been released.  This document discusses major changes.'
...
## Country Changes in the Index between 2019 and `r end_date`
...
spi_index_2019 <- spi_index_df %>%
  ...
  select(iso3c, region, SPI.INDEX.2019)
...
#correlation
corr_end_date_2019 <- cor(spi_changes$SPI.INDEX.end_date,spi_changes$SPI.INDEX.2019, use='pairwise.complete.obs')


```

In order to assess how much change exists in the index values are over time, comparisons have been made between the index values in 2019 and the `r end_date` values. Overall, the SPI overall score is quite stable over time. The correlation between the 2019 value and the `r end_date` value is `r round(corr_end_date_2019,2)`.

*Figure 7.4: Scatterplot of `r end_date` SPI overall score  & 2019 SPI overall score*
```{r changesplot}

ggplot(spi_changes, aes(x=SPI.INDEX.2019, y=SPI.INDEX.end_date, color=region)) +
  ...
  theme_spi() +
  labs(
    title=paste0('Scatterplot of ',end_date,' SPI overall score  & 2019 SPI overall score'),
...
top_changes <- spi_changes %>%
  mutate(changes=SPI.INDEX.end_date-SPI.INDEX.2019) %>%
  arrange(-abs(changes)) %>%
  select(country,SPI.INDEX.end_date,SPI.INDEX.2019, changes) %>%
  mutate(across(c('SPI.INDEX.end_date','SPI.INDEX.2019', 'changes'),round,1))
...
While the scores were relatively stable over time, some countries did see large improvements in their score from 2019-`r end_date`. The country that improved most on the index from 2019 to `r end_date` was `r top_mover`. `r top_mover` improved by `r top_mover_change` points out of 100. The table below shows the changes in the SPI overall score for the top 10 largest improvers.


*Table 7.2: Top 10 Countries with Largest Changes from 2019-`r end_date`.*
...
title <- paste0("Changes in SPI Overall Scores between 2019 and ",end_date,".")

#create dataset with changes between end_date and 2019
  map_df <- spi_changes %>%
    select(iso3c, SPI.INDEX.end_date, SPI.INDEX.2019) %>%
    right_join(country_metadata)
    
  SPI_map <- map_df %>%
    mutate(spi_groups=case_when(
      SPI.INDEX.end_date-SPI.INDEX.2019 > 5 ~ "Improved >5 points",
      between(SPI.INDEX.end_date-SPI.INDEX.2019, 2,5) ~ "Improved 2-5 points",
      between(SPI.INDEX.end_date-SPI.INDEX.2019, -2,2) ~ "Little Change",
      between(SPI.INDEX.end_date-SPI.INDEX.2019, -5,-2) ~ "Dropped 2-5 points",
      SPI.INDEX.end_date-SPI.INDEX.2019 < -5 ~ "Dropped >5 points"
...
growth_plot <- function(variables, name) {
  ...
  elephant_df <- spi_index_df %>%
    ...
    ungroup() %>%
    mutate(growth=(spi_data_end_date-spi_data_2019)) %>%
    filter(!(is.na(spi_data_end_date) | is.na(spi_data_2019))) %>%
  ...
  ggplot(elephant_df, aes(x=spi_bins, y=growth, label=paste0(spi_bins,": ",round(growth,1),""))) +
    ...
    ylab(str_wrap('Change in Score (2019-22)',20)) +
    labs(
      #title=str_wrap("2nd & 3rd deciles have improved most since 2019.",70),
      subtitle=str_wrap('Change in SPI Overall Score from 2019-22 by 2019 decile group',70),
...
```{r elephantstacked, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
  ...
  elephant_stacked_df <- spi_index_df %>%
    ...
    ungroup() %>%
    mutate(overall_growth=(SPI.INDEX_end_date-SPI.INDEX_2019),
           dim1_growth=(SPI.INDEX.PIL1_end_date-SPI.INDEX.PIL1_2019),
           dim2_growth=(SPI.INDEX.PIL2_end_date-SPI.INDEX.PIL2_2019),
           dim3_growth=(SPI.INDEX.PIL3_end_date-SPI.INDEX.PIL3_2019),
           dim4_growth=(SPI.INDEX.PIL4_end_date-SPI.INDEX.PIL4_2019),
           dim5_growth=(SPI.INDEX.PIL5_end_date-SPI.INDEX.PIL5_2019)
           ) %>%
    filter(!(is.na(SPI.INDEX_end_date) | is.na(SPI.INDEX_2019))) %>%
