...
eap_list <- country_metadata %>%
  filter(region=="East Asia & Pacific")

eap_list <- eap_list$country
 ...
 indicator<-indicator
  ...
  region_SPI_df <- region_SPI_df %>%
   ...
   p2 <- ggplot(region_SPI_df, aes(x=Score, y=region, fill=region)) +
      ...
      scale_fill_manual( values = c( "Latin America & Caribbean "="grey",
                                     "South Asia"="grey",
                                     "Sub-Saharan Africa "="grey",
                                     "Europe & Central Asia"="grey",
                                     "Middle East & North Africa"="grey",
                                     "East Asia & Pacific"="tomato",
                                     "North America"="grey"), guide = FALSE ) +
     theme_bw() +
...
p <- ggplot(df_country_time, aes(x=Country)) +
        ...
        ylab('SPI Value') +
        labs(
          title=str_wrap(paste0('Change in ', title, ' from ',start_date,' to ', end_date),90)
...
.center3[
```{r changesplot, echo=FALSE, fig.height=9, fig.width=12, message=FALSE, warning=FALSE, dpi=250}
...
---
.center3[
```{r changesplot2, echo=FALSE, fig.height=9, fig.width=12, message=FALSE, warning=FALSE, dpi=250}
...
```{r eapoverperformers, echo=FALSE, fig.height=9, fig.width=12, message=FALSE, warning=FALSE}
...
gdp_overperformers_plot_eap <- gdp_df_2019 %>%
  modelr::add_residuals(m_spi) %>%
  arrange(-resid) %>%
  filter(country %in% eap_list) %>%
  arrange(resid) %>%
  filter(!is.na(resid))



gdp_overperformers_plot_eap <-  gdp_overperformers_plot_eap %>%
  mutate(country=factor(country, levels=unique(gdp_overperformers_plot_eap$country)),
  ...
  geom_point(size=5) +
  scale_color_manual(name="Country Type",
...
hci_overperformers_plot_eap <- hci_df_2018 %>%
...
hci_overperformers_plot_eap <- hci_overperformers_plot_eap %>%
  mutate(country=factor(country, levels=unique(hci_overperformers_plot_eap$country)),
         group=if_else(resid>=0, 'above','below')) %>%
  ggplot(aes(x=country, y=resid, color=group)) +
  geom_segment( aes(x=country ,xend=country, y=0, yend=resid), color="black") +
  geom_point(size=5) +
...
gdp_overperformers_plot_eap  + hci_overperformers_plot_eap +
  plot_annotation(title='EAP Over/Under-Performers on SPI Index Compared to GDP per capita & Human Capital Index (HCI)',
...
.reduced[
```{r tab1, echo=FALSE, message=FALSE, warning=FALSE, out.height=20, out.width=9}
...
index_tab <- SPI %>%
 ...
 #calculate the breaks for the color coding
      ...
      index_tab <- index_tab %>%
        arrange(country) %>%
        flextable() %>%
        add_header_lines('SPI overall score in 2019 and Pillar Scores for EAP Borrowing Countries.') %>%
        set_header_labels(values=list(
                             country="Country",
                             SPI.INDEX="SPI overall score",
                             SPI.INDEX.PIL1="Data Use",
                             SPI.INDEX.PIL2="Data Services",
                             SPI.INDEX.PIL3="Data Products ",
                             SPI.INDEX.PIL4="Data Sources",
                             SPI.INDEX.PIL5="Data Infrastructure"
                                         )) %>%
          fontsize(size=10) %>%
          bg(j = c('SPI.INDEX'),
