...
## Correlation between the SPI Dimensions
...
```{r zipper, fig.height=12}
 ...
 # create dataframe by income
      ...
      zipper_plt<-ggplot(df_aggregated, aes(x=Country, y=SPI.INDEX)) +
        ...
        #ggtitle(str_wrap("Decomposition of Statistical Performance Indicator Scores by Dimension", 120)) + 
        # scale_x_continuous(
...
```{r decompstacked, echo=FALSE}
  ...
  decomp_stacked_df <- decomp_stacked_df %>%
    ...
    pivot_longer(
      cols=c('SPI.INDEX.DIM1', 'SPI.INDEX.DIM2', 'SPI.INDEX.DIM3', 'SPI.INDEX.DIM4', 'SPI.INDEX.DIM5'),
      values_to='level',
      names_to='dimension'
    ) %>%
    mutate(dimension=case_when(
      dimension=="SPI.INDEX.DIM1" ~ "Dimension 1: Data Use",
      dimension=="SPI.INDEX.DIM2" ~ "Dimension 2: Data Services",
      dimension=="SPI.INDEX.DIM3" ~ "Dimension 3: Data Products",
      dimension=="SPI.INDEX.DIM4" ~ "Dimension 4: Data Sources",
      dimension=="SPI.INDEX.DIM5" ~ "Dimension 5: Data Infrastructure"
    )) %>%
    group_by(income) %>%
    mutate(level=level/5,
           total=sum(level)) #divide by 5 so that dimension scores sum to overall score.  This puts equal weight on each dimension in the sum
...
```{r decompstackedplt, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
    ...
    income <- c("Low income", "Lower middle income","Upper middle income","High income")
#absolute graph
  ggplot(decomp_stacked_df, aes(x=income, y=level, fill=dimension, label=paste0(round(level,1)))) +
    ...
    labs(
      #title='Contribution of each dimension to SPI overall score',
      subtitle='Each dimension receives equal 1/5th weight in overall score',
...
#relative graph  
  ggplot(decomp_stacked_df, aes(x=income, y=100*level/total, fill=dimension, label=paste0(round(100*level/total,1),"%"))) +
    geom_bar(stat = "identity", position='stack') +
    geom_text(size = 4, position = position_stack(vjust = 0.5)) +
    #geom_text(aes(y=total,label=paste0('Total = ' ,round(total,1))), size=4, nudge_y=7 ) +
    scale_x_discrete() +
...
changes_fun('SPI.INDEX.DIM1', 'SPI Dimension 1 (Data Use) Score', 2016, 2019)
...
changes_fun('SPI.INDEX.DIM3', 'SPI Dimension 3 (Data Products) Score', 2016, 2019)

```

```{r changesplotpovind, fig.height=12}

changes_fun('SPI.D3.1.POV', 'SPI Dimension 3 (Data Products) - SDG 1 Score', 2004, 2019)

```
```{r changesplot4, fig.height=12}

changes_fun('SPI.INDEX.DIM4', 'SPI Dimension 4 (Data Sources) Score', 2016, 2019)

```

```{r changesplot4house, fig.height=12}

changes_fun('SPI.D4.1.4.HOUS', 'SPI Dimension 4 - Household Survey Availability - Score', 2010, 2019)

```

```{r changesplot5, fig.height=12}

changes_fun('SPI.INDEX.DIM5', 'SPI Dimension 5 (Data Infrastructure) Score', 2016, 2019)
...
## Figure A.1: Dimension 1 Scores 

### Panel A: Global Map
 ![](plots/dimensionsplot1-1.png)

### Panel B: By Country
  ![](plots/countrydimensionsplot1-1.png)
## Figure A.2: Dimension 2 Scores 

### Panel A: Global Map
  ![](plots/dimensionsplot2-1.png)
### Panel B: By Country
   ![](plots/countrydimensionsplot2-1.png)
## Figure A.3: Dimension 3 Scores

### Panel A: Global Map
  ![](plots/dimensionsplot3-1.png)
### Panel B: By Country
   ![](plots/countrydimensionsplot3-1.png)
## Figure A.4: Dimension 4 Scores

### Panel A: Global Map
  ![](plots/dimensionsplot4-1.png)
### Panel B: By Country
   ![](plots/countrydimensionsplot4-1.png)
## Figure A.5: Dimension 5 Scores

### Panel A: Global Map   
  ![](plots/dimensionsplot5-1.png)
### Panel B: By Country 
   ![](plots/countrydimensionsplot5-1.png)
