...
ggplot(simp_plot_df, aes(x=Original, y=Unweighted)) +
  ...
  theme_bw() +
  labs(
    title="Comparison of SPI Overall Scores and unweighted SPI scores in 2019."
  )
```
...
SPI_2019_simple <- spi_index_simple %>%
  filter(date==2019) %>%
  filter(!is.na(SPI.INDEX))

unique_scores <-  length(unique(SPI_2019_simple$SPI.INDEX))

#check unique values by pillar.
unique_scores1 <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL1))
unique_scores2 <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL2))
unique_scores3 <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL3))
unique_scores4 <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL4))
unique_scores5 <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL5))

#simple index
unique_scores_simp <-  length(unique(SPI_2019_simple$SPI.INDEX.SIMP))


unique_scores1_simp <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL1.SIMP))
unique_scores2_simp <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL2.SIMP))
unique_scores3_simp <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL3.SIMP))
unique_scores4_simp <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL4.SIMP))
unique_scores5_simp <-  length(unique(SPI_2019_simple$SPI.INDEX.PIL5.SIMP))
...
Below we show a measure of volatility over time in a countries Unweighted SPI or SPI Overall Score.  The volatility measure is the standard deviation of the measure from 2016-19.  The Unweighted SPI has slightly greater volatility across time with a global average standard deviation of `r round(global_sd_spi_simp,1)`.  The SPI Overall Score has a global average standard deviation across time of `r round(global_sd_spi_simp,1)`

### Correlation with the other human capital, governance, poverty, and inequality indexes
*Table A.6: Scatterplot of Statistical Capacity Index (SCI) and SPI Overall Score on Log GDP*
```{r scispiregtab1}
...
reg_df <- wbstats::wb_data(
  ...
  left_join(select(spi_index_simple,iso3c,date, SPI.INDEX,SPI.INDEX.SIMP)) #merge on SPI Index data
  %>%
  filter(date==2019)
