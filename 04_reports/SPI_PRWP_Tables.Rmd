...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.height = 6, fig.cap = "", fig.path = "plots/", dev = c("png", "tiff"), dpi=500)
library(tidyverse)
library(flextable)
library(here)
...
## SPI Overall Scores by Quintile in 2020

![](plots/map-1.png)



## SPI Overall Scores by Income Group in 2020
...
## SPI Overall Scores by Lending Type in 2020
...
## SPI Overall Scores by FCS Status in 2020
...
## Correlation between the SPI Pillars in 2020
...
hci_spi <- ggplot(data=hci_df_2018, aes(x=value, y=SPI.INDEX)) +
  ...
  labs(caption="Source: All indicators come from the World Bank. SPI and HCI data are for the year 2020.")
...
pov_df_2020 <- pov_df %>%
  filter(date==2020) 
eq <- lm_robust(SPI.INDEX ~ value, data=pov_df_2020, se_type='HC2')
coef <- coef(eq)
std_err <- sqrt(diag(vcov(eq)))
r_2<- summary(eq)$r.squared
eq_txt <- sprintf("y = %.3f + %.3f x, R^2 = %.3f \n     (%.3f)   (%.3f)", coef[1], coef[2], r_2[1], std_err[1], std_err[2])
 # ggplot(data=pov_df_2020, aes(x=value, y=SPI.INDEX)) +
 #  facet_wrap(~indicator) +
 #  geom_text(aes(label=iso3c)) +
 #  geom_smooth(method='lm') +
 #  theme_spi() +
 ...
 #                                              stat(x_p.value)))) +
 #    theme(legend.position = 'bottom')
pov_spi <- ggplot(data=pov_df_2020, aes(x=value, y=SPI.INDEX)) +
  facet_wrap(~indicator) +
  geom_text(aes(label=iso3c)) +
  geom_smooth(method='lm') +
  theme_spi() +
  ...
  xlab('Poverty Headcount Ratio at $1.90 a day') +
  geom_richtext(
    aes(x = 20, y = 5,label = eq_plot_txt(pov_df_2020), hjust=0.2)
  ) +
...
gini_df <- wbstats::wb_data(
  ...
  left_join(select(spi_index_df,iso3c,date, region,starts_with('SPI.INDEX'))) #merge on SPI Index data
#get the correlation for 2020
gini_df_2020 <- gini_df %>%
  filter(date==2020) 
  # gini_spi <- ggplot(data=gini_df_2020, aes(x=value, y=SPI.INDEX)) +
  # facet_wrap(~indicator) +
  # geom_text(aes(label=iso3c)) +
  # geom_smooth(method='lm') +
  # theme_spi() +
...
gini_spi <- ggplot(data=gini_df_2020, aes(x=value, y=SPI.INDEX)) +
  ...
  xlab('Gini Index')  +
  geom_richtext(
    aes(x = 40, y = 5,label = eq_plot_txt(gini_df_2020), hjust=0.2)
  ) +
    theme(legend.position = 'bottom')
```
...
pov_spi + gini_spi   +
  plot_annotation(
    ...
    caption='Source: All indicators come from the World Bank. SI.POV.DDAY & SI.POV.GINI. SPI data for year 2020. \n Poverty and Gini values are latest values available for countries.'
    )
...
```{r decompstackedplt, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
    ...
    income <- c("Low income", "Lower middle income","Upper middle income","High income")
#absolute graph
  p1 <- ggplot(decomp_stacked_df, aes(x=income, y=level, fill=Pillar, label=paste0(round(level,0)))) +
    geom_bar(stat = "identity", position='stack') +
    geom_text(size = 4, position = position_stack(vjust = 0.5)) +
    geom_text(aes(y=total,label=paste0('Total = ' ,round(total,0))), size=4, nudge_y=7 ) +
    scale_x_discrete() +
    theme_spi() +
...
#relative graph  
  p2 <- ggplot(decomp_stacked_df, aes(x=income, y=100*level/total, fill=Pillar, label=paste0(round(100*level/total,0),"%"))) +
    geom_bar(stat = "identity", position='stack') +
    geom_text(size = 4, position = position_stack(vjust = 0.5)) +
...
growth_plot <- function(variables, name) {
  ...
  ggplot(elephant_df, aes(x=spi_bins, y=growth, label=paste0(round(growth,1),""))) +
    ...
    ylab(str_wrap('Change in Score (2016-20)',20)) +
    labs(
      title=str_wrap("Countries in 2nd and 3rd deciles have grown most since 2016.",70),
      subtitle=str_wrap('Change in SPI Overall Score from 2016-20 by 2016 decile group',70),
      caption=paste0(name,' scale = 0 - 100 points.')
    ) +
...
changes_fun('SPI.D1.5.POV', 'SPI Comparable Poverty Data Indicator', 2004, 2020)
```

```{r changesplot1, fig.height=12}
changes_fun('SPI.INDEX.PIL1', 'SPI Pillar 1 (Data Use) Score', 2016, 2020)
```
...
changes_fun('SPI.INDEX.PIL3', 'SPI Pillar 3 (Data Products) Score', 2016, 2020)
```

```{r changesplotpovind, fig.height=12}
changes_fun('SPI.D3.1.POV', 'SPI Pillar 3 (Data Products) - SDG 1 Score', 2004, 2020)
```
```{r changesplot4, fig.height=12}
changes_fun('SPI.INDEX.PIL4', 'SPI Pillar 4 (Data Sources) Score', 2016, 2020)
```

```{r changesplot4house, fig.height=12}
changes_fun('SPI.D4.1.4.HOUS', 'SPI Pillar 4 - Household Survey Availability - Score', 2010, 2020)
```

```{r changesplot5, fig.height=12}
changes_fun('SPI.INDEX.PIL5', 'SPI Pillar 5 (Data Infrastructure) Score', 2016, 2020)
```
# Tables

## Country SPI overall scores

Below, we present the full list of countries by their SPI overall score in 2020.  The first column is the country name and the following columns are the overall SPI overall score, and then the sub-scores for Pillar 1,2,3,4 and 5.  
...
## Figure A.1: Pillar 1 Scores in 2020
...
The SCI contains `r unique_scores_sci` unique scores for `r length(unique_scores_df$SCI)` countries in 2020.
...
Below we show a measure of volatility over time in a countries SCI or SPI Overall Score.  The volatility measure is the standard deviation of the measure from 2016-20.  The SCI has slightly greater volatility across time with a global average standard deviation of `r round(global_sd_sci,1)`.  The SPI Overall Score has a global average standard deviation across time of `r round(global_sd_spi,1)`
...
ggplot(simp_plot_df, aes(x=Original, y=Unweighted)) +
  ...
  theme_bw() +
  labs(
    title="Comparison of SPI Overall Scores and unweighted SPI scores in 2020."
  )
```
...
SPI_2020_simple <- spi_index_simple %>%
  filter(date==2020) %>%
  filter(!is.na(SPI.INDEX))

unique_scores <-  length(unique(SPI_2020_simple$SPI.INDEX))

#check unique values by pillar.
unique_scores1 <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL1))
unique_scores2 <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL2))
unique_scores3 <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL3))
unique_scores4 <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL4))
unique_scores5 <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL5))

#simple index
unique_scores_simp <-  length(unique(SPI_2020_simple$SPI.INDEX.SIMP))


unique_scores1_simp <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL1.SIMP))
unique_scores2_simp <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL2.SIMP))
unique_scores3_simp <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL3.SIMP))
unique_scores4_simp <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL4.SIMP))
unique_scores5_simp <-  length(unique(SPI_2020_simple$SPI.INDEX.PIL5.SIMP))
...
Below we show a measure of volatility over time in a countries Unweighted SPI or SPI Overall Score.  The volatility measure is the standard deviation of the measure from 2016-20.  The Unweighted SPI has  a global average standard deviation of `r round(global_sd_spi_simp,1)`.  The SPI Overall Score has a global average standard deviation across time of `r round(global_sd_spi,1)`
