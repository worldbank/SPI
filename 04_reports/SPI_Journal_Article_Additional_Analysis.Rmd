...
SPI_2016 <- SPI %>%
  filter(date>=2016) %>%
  group_by(iso3c) %>%
  fill(c( 'SPI.INDEX'), .direction="downup") %>%
  ungroup() %>%
...
#create regression models
 ...
 'Longitudinal Model (Region FE/Year FE)' = feols(log(NY.GDP.PCAP.KD) ~  SPI.INDEX + NV.IND.MANF.ZS + NV.AGR.TOTL.ZS + SE.PRM.ENRR  + NE.TRD.GNFS.ZS + WGI.OVL   | date + region, data=reg_df, se='cluster', cluster='country') ,
...
fixed <- plm::plm(log(NY.GDP.PCAP.KD) ~   SPI.INDEX + NV.IND.MANF.ZS + NV.AGR.TOTL.ZS + SE.PRM.ENRR  + BN.CAB.XOKA.GD.ZS + WGI.OVL + factor(date), data=reg_df, model='within', index=c('iso3c'))

summary(fixed)

random <- plm::plm(log(NY.GDP.PCAP.KD) ~   SPI.INDEX + NV.IND.MANF.ZS + NV.AGR.TOTL.ZS + SE.PRM.ENRR  + BN.CAB.XOKA.GD.ZS + WGI.OVL + factor(date) , data=reg_df, model='random', index=c('iso3c'))
