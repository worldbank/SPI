...
labels <- metadata_info[match(colnames(spi_index),metadata_info$source_id),2]
labels <- labels$descript
names(labels) <-colnames(spi_index)
  
label(spi_index) <- as.list(labels)
```
...
vtable((spi_index %>% select(-ISO_A3_EH)),
       out='return') %>%
  mutate(Class=if_else(Name %in% c("country", "iso3c", "income", "region"), "character", "numeric")) %>%
  flextable() %>%
  theme_vanilla() %>%
  add_header_lines('Data Description of SPI_index.csv')
...
The data on 10 year dollar denominated government bond spreads was collected from Bloomberg from the period 2015 to 2020 for `r country_num_bonds`.^[The countries where bond spreads were collected includes `r country_list_bonds`.]
