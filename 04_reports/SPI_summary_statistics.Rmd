...
spi_mapper <- function(data, indicator, title) {
 ...
 indicator<-indicator
  ...
  spi_groups_quantiles <- quantile(map_df$data_available, probs=c(1,2,3,4,5)/6,na.rm=T)
  
  SPI_map <- map_df %>%
    mutate(spi_groups=case_when(
      between(data_available, spi_groups_quantiles[5],100) ~ "High-top",
      between(data_available, spi_groups_quantiles[4],spi_groups_quantiles[5]) ~ "High-bottom",
      between(data_available, spi_groups_quantiles[3],spi_groups_quantiles[4]) ~ "Middle-top",
      between(data_available, spi_groups_quantiles[2],spi_groups_quantiles[3]) ~ "Middle-bottom",
      between(data_available, spi_groups_quantiles[1],spi_groups_quantiles[2]) ~ "Low-top",
      between(data_available, 0,spi_groups_quantiles[1]) ~ "Low-bottom"
      
    )) %>%
    mutate(spi_groups=factor(spi_groups, 
                             levels=c("High-top","High-bottom","Middle-top","Middle-bottom","Low-top", "Low-bottom"  )))  
  
  #set color pallete
  col_pal <- c("#1A9850",  "#66BD63", "#E4ED40", "#FFFF8C",  "#FC7F60","#D73027" )
  names(col_pal) <- c("High-top","High-bottom","Middle-top","Middle-bottom","Low-top", "Low-bottom"  )
