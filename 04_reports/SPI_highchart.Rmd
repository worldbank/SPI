...
```{r old, echo=FALSE}
 ...
 country_report_lolli_fn <- function(variables, title, scale) {
      ...
      lollip_df_temp <- lollip_df_temp %>%
        ...
        select(shortname, values, dimname, country, source_name,source_id ) %>%
...
spi_2 <- country_report_lolli_fn('SPI.INDEX','Overall Index', 100) %>%
  ...
  nest(-country) %>%
    mutate(
      data = map(data, mutate_mapping, hcaes(x=shortname, y = values)),
      data = map(data, list_parse)
    )
