...
The SCI (in use since 2004) has three dimensions:   
  1. Methodology (Balance of payments manual in use, CPI base year, External debt reporting status, Government finance accounting, import/export price indexes, industrial producting index, national accounts base year, national immunization coverage, SDDS, UNESCO reporting)   
...
---
.center3[
```{r overall_charts, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
Dimension 2: Data Services - Scores Across Countries
...
---
.center2[
```{r dimension_2_map, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
---
.center3[
```{r dimension_2_country_charts, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
spi_country_charts('SPI','SPI.INDEX.DIM2','Statistical Performance Indicators -  SPI Dimension 2: Data Services')
...
Dimension 5: Data Services - Scores Across Countries

---
.center2[
```{r dimension_5_map, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
---
.center3[
```{r dimension_5_country_charts, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
spi_country_charts('SPI','SPI.INDEX.DIM5','Statistical Performance Indicators -  SPI Dimension 5: Data Infrastructure')
...
* Includes 11 Indicators:
  ...
  2. System of national accounts in use   
  3. National Accounts base year	    
  4. <span style="color:green"> Classification of national industry	    
  5. <span style="color:green"> CPI base year	    
  6. <span style="color:green"> Classification of household consumption (COICOP)	    
  7. <span style="color:green"> Classification of status of employment	    
  8. <span style="color:green"> Central government accounting status (AC/CA)	    
  9. <span style="color:green"> Compilation of government finance statistics (GFSM)	    
  10. <span style="color:green"> Compilation of monetary and financial statistics (MFS)	    
  11. Business process	    
...
```{r sumstats_table, echo=FALSE, dpi=250, out.height=12, out.width=10}
...
---
.center3[
```{r dim_corr, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=9, fig.width=12}

indicators<-c("SPI.INDEX",
              "SPI.INDEX.DIM5",
              "SPI.D2.1.GDDS",
              "SPI.D5.2.1.SNAU",
              "SPI.D5.2.2.NABY",
...
#attach labels
labels_df <- metadata %>%
  filter(series %in% indicators) %>%
  arrange(factor(series, levels=indicators))

colnames(df_corr_plot) <- labels_df$indicator_name
rownames(df_corr_plot) <- labels_df$indicator_name

#plot the correlation in a nicely formatted table
pcorr<- ggcorrplot::ggcorrplot(df_corr_plot[,13:1],
                   outline.color = "white",
                   ggtheme = theme_bw(),
...
pcorr +
  theme(
    ...
    axis.text.x = element_text(angle=0,vjust=.5, size=7,
                               face = ifelse((levels(pcorr$data$Var1) %in% c('Compilation of monetary and financial statistics')) , "bold", "plain")),
    axis.title = element_blank(),
    axis.text.y = element_text(face = ifelse((levels(pcorr$data$Var2) %in% c('Compilation of monetary and financial statistics')) , "bold", "plain")
                               )
  ) +
  labs( )
...
.center3[
```{r trends, echo=FALSE, dpi=250, fig.height=10, fig.width=12, message=FALSE, warning=FALSE, dpi=300}
...
```{r spi_map, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
```{r standards_meta, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
```{r mony, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center2[
```{r maps_5.2.9, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center3[
```{r charts_5.2.9, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
D5.2.9.FSI.IMF <-   read_excel(path=paste(raw_dir, "5.2_DISM","2020.11.04_Compiled Data for World Bank.xlsx", sep="/" ),
  ...
  mutate(SPI.D5.2.9.FSI.IMF=case_when(
    grepl('2019|2020', Financial.Soundness.Indicators..Core.Set..Deposit.Takers..Asset.Quality..Non.performing.Loans.to.Total.Gross.Loans..Percent) ~ 1, 
    grepl('2018', Financial.Soundness.Indicators..Core.Set..Deposit.Takers..Asset.Quality..Non.performing.Loans.to.Total.Gross.Loans..Percent) ~ 0.5, 
    TRUE ~ 0 
  ),
...
D5.2.9.FSI.IMF_annual <-   read_excel(path=paste(raw_dir, "5.2_DISM","2020.11.04_Compiled Data for World Bank.xlsx", sep="/" ),
                               sheet ="FSI Annual",
                               skip=1
                               ) %>%
  as_tibble(.name_repair='universal') %>%  
  ...
  mutate(IMF_code=as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  select(iso3c ,SPI.D5.2.9.FSI.IMF_annual ) %>%
  mutate(date=2019)
...
D5.2.9.FSI <-  D5.2.9.FSI.IMF %>%
  ...
  rowwise() %>%
  mutate(SPI.D5.2.9.FSI=max(c_across(c('SPI.D5.2.9.FSI.IMF','SPI.D5.2.9.FSI.IMF_annual')) )) %>% #fill in with IMF data if missing.  This applies mostly to high income countries
  select(-SPI.D5.2.9.FSI.IMF,-SPI.D5.2.9.FSI.IMF_annual) %>%
  arrange(date, iso3c)
...
---
.center2[
```{r maps_2.1, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
```{r gdds, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
#####
# IMF Data
#####
...
.center2[
```{r maps_2.1b, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center3[
```{r charts_2.1, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
```{r cpiby, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center2[
```{r maps_5.2.4, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center3[
```{r charts_5.2.4, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
```{r hous, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
D5.2.5.HOUS <-  D5.2.5.HOUS %>%
  ...
  mutate(SPI.D5.2.5.HOUS=if_else(is.na(SPI.D5.2.5.HOUS.IMF),SPI.D5.2.5.HOUS,SPI.D5.2.5.HOUS.IMF)) %>% #fill in with manually collected data if missing.  This applies mostly to high income countries
...
.center2[
```{r maps_5.2.5, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center3[
```{r charts_5.2.5, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
```{r cgov, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center2[
```{r maps_5.2.7, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
.center3[
```{r charts_5.2.7, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
.center3[
```{r charts_5.2.9.fsi, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
