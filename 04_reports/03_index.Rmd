...
hci_spi <- ggplot(data=hci_df_2018, aes(y=value, x=100*SPI.INDEX)) +
...
gdp_df_2019 <- gdp_df %>%
  ...
  gdp_spi <- ggplot(data=gdp_df_2019, aes(y=value, x=100*SPI.INDEX)) +
  facet_wrap(~indicator) +
  geom_text(aes(label=iso3c)) +
  geom_smooth(method='lm') +
...
index_tab <- index_tab %>%
  ...
  set_header_labels(values=list(
    ...
    bg(j = c('SPI.INDEX'), 
       bg = scales::col_quantile("RdYlGn", domain=NULL, n=8)) %>%
    bg(j = c('SPI.INDEX.DIM1'),
       bg = scales::col_quantile("RdYlGn", domain=NULL, n=4)) %>%
