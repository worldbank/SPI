...
Our statistical performance indicators have a three level structure, and our SPI Index score will be formed by sequentially aggregating each level.
...
Where SPI.INDEX is the SPI index. SPI.DIM are the 5 SPI dimensions listed above. In the notation, c is a country, t is the date, d is a dimension.  
...
* **Most Mature**:  Countries in the top 20% are classified in this group.  Shading in <span style="color:#1A9850">green</span>.    
* **Highly Mature**: Countries in the 4th quantile, or those above the 60th percentile but below the 80th percentile are in this group.  Shading in <span style="color:#BFCF28"> light green</span>.    
* **Maturing**: Countries in the 3rd quantile, or those between the 40th and 60th percentile, are classified in this group.  Shading in <span style="color:#E7BB25"> yellow</span>.  
* **Less Mature**: Countries in the 2nd quantile, or those above the 20th percentile but below the 40th percentile, are in this group.  Shading in <span style="color:#DE7B2B"> orange</span>.  
* **Least Mature**: Countries in the bottom 20% are classified in this group.  Shading in <span style="color:#D73027"> red </span>.   


```{r map}
#show map and summary stats by region and income group

spi_mapper('spi_index_df','SPI.INDEX','SPI Index')
...
The maturity model reflects concrete differences between countries in their statistical systems.  On the SPI index itself, the countries scoring in the top 20% have an average SPI index score of `r spi_top`.  A maximum score of 100 would indicate that a country has every single element in place that we measure through our indicators.  Meanwhile, countries in the bottom 20% score significantly worse, with an average index scores of `r spi_bottom`.  To highlight a few specific indicators, we can also see that countries in the top 20% on average get nearly perfect scores in terms of the debt reporting to the world bank.  The top 20% of countries in the SPI index have an average score on their debt reporting of `r debt_top`, with a maximum score of 1 and minimum score of 0, while those in the bottom 20% on the SPI index have an average score of `r debt_bottom`.  To take another concrete example, when looking at scores on whether a population census has been conducted recently, countries in the top 20% score on average `r census_top` on our population census indicator, with a max score of 1 and minimum score of 0, while those in the bottom 20% on our SPI index indicator score `r census_bottom` on the population census indicator.  The table below shows differences across maturity groups for a select set of indicators.
...
## SPI Index by Region and Income Group

There are large differences in the SPI index across World Bank regions and income groups.  Overall, North America has the highest average SPI index, while the Sub-Saharan Africa region has the lowest average score.^[All tables and figures show unweighted summary statistics (i.e. the summary statistics do not weight by population).] There is also a clear gradient with respect to income groups.  Countries classified as low income have lower scores on average than countries classified as middle income.  High income countries have the highest average SPI index score.  

Finally, when looking over time, since 2016 the SPI index values are relatively stead across regions.  There has been some modest increase in the SPI index scores for middle income and high income countries, with little progress for low income countries.

Figure 5.2.
```{r spi_charts}
#show map and summary stats by region and income group

spi_charts('spi_index_df','SPI.INDEX','SPI Index')
...
While there are large differences across regions, within regions there is significant variation in the SPI index.  For instance, in the Latin America & Caribbean World Bank region, `r top_lac` is the highest scoring country on the SPI index with a score of `r top_lac_score`.  However, `r bottom_lac`, the lowest scoring country in the region, earns a substantially lower score of `r bottom_lac_score`.  In Sub-Saharan Africa, the highest scoring country is `r top_ssa` with a score of `r top_ssa_score`, while the lowest scoring country is `r bottom_ssa` with a score of `r bottom_ssa_score`.  In the East Asia and Pacific region, the top scoreing country is `r top_eap` with a score of `r top_eap_score`, while the lowest scoring country is `r bottom_eap` with a score of `r bottom_eap_score`.

Figure 5.3
```{r spicountrycharts}
#show map and summary stats by region and income group

spi_country_charts('spi_index_df','SPI.INDEX','SPI Index')
...
indicators<-c("SPI.INDEX.DIM1","SPI.D1.5.POV","SPI.D1.5.CHLD.MORT","SPI.D1.5.DT.TDS.DPPF.XP.ZS")
...
The table below shows the average score on each indicator in Dimension 1 by maturity group.  Countries in the top 20% possess nearly all of the components in Dimension 1, while countries in the bottom 20%.  Countries in the bottom 20% in terms of maturity on Dimension 1 have an average of `r poverty_bottom` on the comparable poverty estimate indicator, an average of `r mort_bottom` on the availability of under 5 mortality data, and an average of `r debt_bottom` on debt reporting.
...
In the following chart, we show the correlations between the SPI index and the SPI dimension sub-scores.  All dimensions are positively correlated with one another.  On the other hand, no dimension is perfectly correlated with any of the other dimensions, which would be a sign that a dimension was not provide additional information on the statistical performance of countries.  The dimension with the single highest correlation to the overall measure is dimension 1 on data use.  The indicator with the lowest overall correlation with the SPI index is dimension 3 on data products.
...
spi_odin_plot <- ggplot(comparison_df, aes(y=SPI.INDEX,x=ODIN_score, color=region)) +
  ...
  theme_bw() +
  labs(
    title='Scatterplot of SPI Index on Open Data Watch Index'
  ) +
  ylab('SPI Index value') +
  xlab('ODIN value') +
  theme(legend.position = 'bottom')
...
spi_spi_plot <- ggplot(comparison_df, aes(y=SPI.INDEX,x=SPI_v0, color=region)) +
  ...
  theme_bw() +
  labs(
    title='Scatterplot of new SPI Index on SPI Version 0 Index'
  ) +
  ylab('SPI Index value') +
...
col_palette2 <- c("#1A9850",  "#E7BB25", "#D73027" )
...
index_tab <- index_tab %>%
  flextable() %>%
  add_header_lines('SPI Index in 2019 and Dimension Scores.') %>%
  set_header_labels(values=list(
                       country="Country", 
    ...
    bg(j = c('SPI.INDEX.DIM1'),
       bg = scales::col_quantile(col_palette2, domain=NULL, n=3, reverse=TRUE)) %>%
