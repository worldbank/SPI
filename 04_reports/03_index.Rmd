...
A quick primer on names. We refer to the 5 rows in the framework in Figure 1 as dimensions. We refer to the 22 cells in the framwork in Figure 1 as indicators. Finally, each indicator may be composed of multiple sub-indicators. For instance, the indicator on censuses and surveys is made up sub-indicators on whether population censuses have been conducted, agriculture censuses, labor force surveys, etc.

Below is the full set of indicators available to populate the index. along with the corresponding sub-indicators:
...
index_tab <- flextable(metatable) %>%
  ...
  set_header_labels(values=list(
                       source_id="ID",
                       source_name="Sub-Indicator Name",
                       SPI_indicator_id="Indicator",
...
The Index will contain only a subset of the sub-dimensions from our framework.  For dimensions excluded, we either lacked a source with a developed methodology or else the data collection for that measure was incomplete. This is described below:

  * **Indicator 1.1: Data use by national legislature:**  *Not included because of lack of established methodology*   
  * **Indicator 1.2: Data use by national executive branch:** *Not included because of lack of established methodology*      
  * **Indicator 1.3: Data use by civil society:**  *Not included because of lack of established methodology*    
  * **Indicator 1.4: Data use by academia:**  *Not included because of lack of established methodology*   
  * **Indicator 1.5: Data use by international organizations:**  *Reliability/Usefulness of Poverty, Child Mortality, Debt Statistics for international agencies using metadata*    

  
  * **Indicator 2.1: Data Releases:** *SPI.D2.1.GDDS -	SDDS/e-GDDS subscription*    
  * **Indicator 2.2: Online access:** *SPI.D2.2.Openness.subscore	ODIN Open Data Openness score*   
  * **Indicator 2.3: Advisory/ Analytical Services:**  *Not included because of lack of established methodology*    
  * **Indicator 2.4: Data services:** *SPI.D2.4.NADA	NADA metadata*
  
  * **Indicator 3.1: Social Statistics:** *Average score for Goal 1-6 indicators*
  * **Indicator 3.2: Economic Statistics:** *Average score for Goal 7-12 indicators*  
  * **Indicator 3.3: Environmental Statistics:**  *Average score for Goal 13-15 indicators*      
  * **Indicator 3.4: Institutional Statistics:** *Average score for Goal 16-17 indicators*    

  * **Indicator 4.1: Censuses and Surveys:** *Average score Census and Survey Indicators indicators SPI.D4.1....*
  * **Indicator 4.2: Administrative Data:** *Average score for CRVS indicator and Education (UNESCO) admin data indicator.  Social Protection and Labor admin data indicators not included because of lack of established methodolgy*  
  * **Indicator 4.3: Geospatial Data:**  *SPI.D4.3.GEO.first.admin.level -	Geospatial data available at 1st Admin Level* 
  * **Indicator 4.4: Private/citizen generated data:**  *Not included because of lack of established methodology*   
  
  * **Indicator 5.1: legislation and governance:**  *Not included because of insufficient country coverage*  
  * **Indicator 5.2: Standards and Methods:** *Average score for Standards and Methods indicators SPI.D5.2....*  
  * **Indicator 5.3: skills:**  *Not included because of lack of established methodology*    
  * **Indicator 5.4: partnerships:**  *Not included because of lack of established methodology*    
  * **Indicator 5.5: finance:**  *Not included because of insufficient country coverage* 
  

This process of eliminating some indicators due to lack of established methodology or country coverage results in 12 indicators. This includes 1 on data use, 3 on data services, 4 on data products, 3 on data sources, and 1 on data infrastructure.
...
To begin we produce a score for each indicator, which is an unweighted average of the sub-indicators within that indicator.  For instance, the Census and Surveys indicator will be formed by taking the unweighted average of the Population Census score, the Agriculture Census score, the Business Census score, the Labor Force Survey score, the Health Survey score, etc.  

$$ SPI.IND_{ctdi} = \sum_{j=1}^{N_J} \frac{SPI.SUB.IND_{ctdij}}{N_J} $$

where $SPI.IND_{ctdi}$ is indicator i, in dimension d, in time period t, and country c.  $SPI.SUB.IND_{ctdim}$ is a sub-indicator (e.g. population census score).

After computing a score for the indicators, we then compute a dimension score, which is the unweighted average of the indicators in that dimension.

$$ SPI.DIM_{ctd} = \sum_{i=1}^{N_I} \frac{SPI.IND_{ctdi}}{N_I} $$
...
spi_index_map <- spi_index_df %>%
  select(country,date, SPI.INDEX)

spi_mapper('spi_index_map','SPI.INDEX','Overall SPI  Index Averaging Over All Sub-Dimensions.  Equal Weight for Each Sub-Dimension.')
...
# create table
####
DT::datatable(index_disp, colnames = c(
                       "Country", 
                       "ISO3C", 
                       "Date", 
                       "SPI Index Value",
              ...
              caption=htmltools::tags$caption(
            style = 'caption-side: top; text-align: left;',
            htmltools::em("Overall SPI  Index Averaging Over All Sub-Dimensions."))    ,
            extensions = 'Buttons', options=list(
                dom = 'Bfrtip',
                buttons = c('copy', 'csv', 'excel' ),
                pageLength = 218)) 
...
As a final comparison, we compare our new SPI index to an index developed by Cameron, Dang, Dinc, Foster, and Lokshin (2019), which can be thought of as a version 0 of our SPI index.  The authors use similar data sources for their index.  However, there are some differences.
...
Cameron,Grant James; Dang,Hai-Anh H.; Dinc,Mustafa; Foster,James Stephen; Lokshin,Michael M..2019.

Measuring the Statistical Capacity of Nations (English). Policy Research working paper;no. WPS 8693;Paper is funded by the Knowledge for Change Program (KCP);Paper is funded by the Strategic Research Program (SRP) Washington, D.C. : World Bank Group. http://documents.worldbank.org/curated/en/304431546956224461/Measuring-the-Statistical-Capacity-of-Nations

http://documents1.worldbank.org/curated/en/304431546956224461/pdf/WPS8693.pdf
...
index_tab <- flextable(index_disp) %>%
