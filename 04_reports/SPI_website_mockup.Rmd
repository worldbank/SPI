...
plot_fun <- function(variables, title) {
  ...
  plot_df <- SPI_2019 %>%
    ...
    spi_groups_quantiles <- quantile(plot_df$value, probs=c(1,2,3,4)/5,na.rm=T)
    
    
    plot_df <- plot_df %>%
      mutate(spi_groups=case_when(
        between(value, spi_groups_quantiles[4],100) ~ "Top 20%",
        between(value, spi_groups_quantiles[3],spi_groups_quantiles[4]) ~ "4th Quintile",
        between(value, spi_groups_quantiles[2],spi_groups_quantiles[3]) ~ "3rd Quintile",
        between(value, spi_groups_quantiles[1],spi_groups_quantiles[2]) ~ "2nd Quintile",
        between(value, 0,spi_groups_quantiles[1]) ~ "Bottom 20%"
  ...
  p <- ggplot(plot_df,aes(income, value, color=spi_groups, label=country)) + 
    geom_quasirandom() +
    scale_color_manual(
        name='',
        values=col_pal,
...
pil1 <- plot_fun('SPI.INDEX.PIL1', 'Pillar 1: Data Use') 


write_excel_csv(pil1,'C:/Users/wb469649/OneDrive - WBG/DECIS/SPI/charts_data/pillar1_data.csv')   
```
