...
Based on feedback, goal was for SPI framework to:  
  - Be forward looking (relevant ~ 10-15 years)   
...
- SPI framework covers five dimensions:
  ...
  3. Data Products
      * Whether the country is able to produce relevant indicators, primarily related to SDGs    
  4. Data Sources
      * Censuses, surveys, admin data, geospatial, private/citizen generated data    
---
 # Framework  Overview
...
.center[![unchanged image](SPI_Framework.png)]
...
spi_charts  <- function(data, indicator, title) {
 ...
 indicator<-indicator
  ...
  region_SPI_df <- map_df %>%
    ...
    summarise(Percentage=wtd.mean(data_available, weights = weights, na.rm=T),
  ...
  region_SPI_df <- region_SPI_df %>%
    ...
    #set color pallete
  # col_pal <- c("#4D86A5","#CF0BF1","#2DFFDF","#3E517A","#98DA1F","#FC9F5B","#D60B2D")  
  # names(col_pal) <- unique(map_df$region)

   p2 <- ggplot(region_SPI_df, aes(x=Percentage, y=region, fill=region)) +
...
lending_charts <- function(data, indicator, title) { 
 ...
 indicator<-indicator
  ...
  lending_list <- c("Not classified", "IBRD", "Blend", "IDA" )
    ...
    #set color pallete
  col_pal <- c("#533A71","#6184D8","#50C5B7","#9CEC5B")  
  names(col_pal) <- lending_list
...
fcs_charts <- function(data, indicator, title) { 
 ...
 indicator<-indicator
  ...
  #set color pallete
  col_pal <- c("#FFBE0B","#FF006E")  
  names(col_pal) <- fcs_list
...
# Index Methodology
  ...
  * Produce overall score combining indicators
  
  * Three level structure:    
    * Indicators nested under pillars (sub-dimensions): $SPI.PILLAR_{ctds} = \sum_{i=1}^{N_I} \frac{SPI.IND_{ctdsi}}{N_I}$  
    * Pillars (sub-dimensions) nested under dimensions: $SPI.DIM_{ctd} = \sum_{s=1}^{N_S} \frac{\omega_{ds} SPI.PILLAR_{ctds}}{N_S}$   
    * Dimensions nested under overall score: $SPI.INDEX_{ct} = \sum_{d=1}^{N_D} \frac{\alpha_{d} SPI.DIM_{ctd}}{N_D}$  
...
Scores by Dimension Across Countries
...
changes_fun('SPI.INDEX.DIM2', 'SPI Dimension 2 - Data Services - Score', 2016, 2019)
...
changes_fun('SPI.D3.1.POV', 'SPI Dimension 3 (Data Products) - SDG 1 Score', 2010, 2019)
...
changes_fun('SPI.D4.1.4.HOUS', 'SPI Dimension 4 - Household Survey Availability - Score', 2009, 2019)
...
.center3[
```{r dimcorr, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=10}
...
colnames(df_corr_plot) <- c('SPI Dimension 1: Data Use','SPI Dimension 2: Data Services','SPI Dimension 3: Data Products','SPI Dimension 4: Data Sources','SPI Dimension 5: Data Infrastructure','SPI Overall')
...
* Statistical Performance Indicators will be released at same time as new World Development Report
...
---
.center3[
```{r region_chartsnohic, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
spi_charts('SPI_noHIC','SPI.INDEX','SPI Overall Scores (excluding High Income)')

```
]
---
.center3[
```{r region_charts_lowpop, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
spi_charts('SPI_lowpop','SPI.INDEX','SPI Overall Scores (excluding High Income & populations less than 1 million)')
