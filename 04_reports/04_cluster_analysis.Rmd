...
dir <- paste(here(), '03_output_data', sep="/")
...
#import data
SPI <- read_csv(paste(dir, 'SPI_index.csv', sep="/")) %>%
    left_join(pop_df) %>%
...
This is some exploratory analysis looking at both the number of pillars in our statistical performance indicators and whether our quintile based approach for grouping countries is sensible.

We apply two methods.  First, we use principal components analysis (PCA) to examine how many relevant dimensions are present across our 51 SPI indicators.  Second, we compare our SPI quintile groups to groups formed using K means clustering.

Takeaways:

* There appears to be a single dominant dimension according to PCA that accounts for roughly 33% of the variation in our indicators.  This primary principal component is very strongly related to our SPI overall score ($R^2=0.97$).  There is also a second dimension that explains around 10% of the variation.  This second dimension seems to correspond most strongly with the Data Products pillar ($R^2=0.33$)
...
pca_df <- spi_index_df %>%
  ...
  select(-starts_with('SPI.INDEX'),-SPI.D5.1.DILG, -SPI.D5.5.DIFI) %>%
...
pca_compare_df2 <- pca_compare_df2 %>%
  ...
  mutate(index=case_when(
    ...
    index=='SPI.INDEX.PIL1' ~ "Pillar 1: Data Use",
    index=='SPI.INDEX.PIL2' ~ "Pillar 2: Data Services",
    index=='SPI.INDEX.PIL3' ~ "Pillar 3: Data Products",
    index=='SPI.INDEX.PIL4' ~ "Pillar 4: Data Sources",
    index=='SPI.INDEX.PIL5' ~ "Pillar 5: Data Infrastructure"
  ))
