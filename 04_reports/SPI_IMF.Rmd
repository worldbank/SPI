---
title: "Statistical Performance Indicators: IMF Presentation"
...
- SPI framework covers five dimensions:
  1. Data Use
      * Indicators that capture demand side of statistical system   
  2. Data Services
      * <span style="color:green"> Information on data releases </span>, online access, advisory/analytical services, and other data services
...
- SPI framework covers five dimensions:
  ...
  5. Data Infrastructure
      * Legislation and governance, <span style="color:green">standards & methods </span>, skill, partnerships, and finance
...
# growth_plot('SPI.INDEX.PIL1', 'SPI Dimension 1 (Data Use) Score')
# growth_plot('SPI.INDEX.PIL2', 'SPI Dimension 2 (Data Services) Score')
# growth_plot('SPI.INDEX.PIL3', 'SPI Dimension 3 (Data Products) Score')
# growth_plot('SPI.INDEX.PIL4', 'SPI Dimension 4 (Data Sources) Score')
# growth_plot('SPI.INDEX.PIL5', 'SPI Dimension 5 (Data Infrastructure) Score')
...
* Framework is meant to be comprehensive & flexible enough to allow customization.    
  ...
  * i.e. instead of a data products dimenions centered on SDGs    
  * Could be based on any set of key indicators relevant to IMF.    
...
# Data Services (4 Pillars)
  ...
  * <span style="color:green"> **Pillar 2.1: Data Releases:** 
...
# Data Infrastructure (5 Pillars) 
  ...
  * <span style="color:green"> **Pillar 5.2: Standards and Methods:** 
...
# Indicators collected by IMF

* Next, will focus on indicators compiled using IMF sources   
* Includes 8 Indicators:
  1. <span style="color:green"> SDDS/e-GDDS subscription	    
  2. <span style="color:green"> Classification of national industry	    
  3. <span style="color:green"> CPI base year	    
  4. <span style="color:green"> Classification of household consumption (COICOP)	    
  5. <span style="color:green"> Classification of status of employment	    
  6. <span style="color:green"> Central government accounting status (AC/CA)	    
  7. <span style="color:green"> Compilation of government finance statistics (GFSM)	    
  8. <span style="color:green"> Compilation of monetary and financial statistics (MFS)		    
  
---

.center2[
```{r dimension_1_m, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
spi_mapper('SPI', 'SPI.INDEX.PIL1', 'Statistical Performance Indicators - SPI Dimension 1: Data Use')
...
spi_mapper('SPI', 'SPI.INDEX.PIL2', 'Statistical Performance Indicators - SPI Dimension 2: Data Services')
...
spi_mapper('SPI', 'SPI.INDEX.PIL3', 'Statistical Performance Indicators - SPI Dimension 3: Data Products')
...
spi_mapper('SPI', 'SPI.INDEX.PIL4', 'Statistical Performance Indicators - SPI Dimension 4: Data Sources')
...
spi_mapper('SPI', 'SPI.INDEX.PIL5', 'Statistical Performance Indicators - SPI Dimension 5: Data Infrastructure')
...
---
# Customizing SPI framework for IMF statistics

* Framework is flexible enough to allow customization. 

* For instance, instead of a data products based on reporting for the SDGs,  could be based on reporting for the the Financial Soundness Indicators (FSIs).

* This indicator checks for the recency of FSI reporting, using data on last quarter that FSI values were updated.
...
# IMF-SPI Dimension 3: Data Products


Examine five indicators:    
  1. Financial Soundness Indicators, Core Set, Deposit Takers, Asset Quality, Non-performing Loans to Total Gross Loans, Percent	    
  2. Financial Soundness Indicators, Core Set, Deposit Takers, Earnings and Profitability, Return on Assets, Percent	    
  3. Financial Soundness Indicators, Core Set, Deposit Takers, Earnings and Profitability, Return on Equity, Percent	    
  4. Financial Soundness Indicators, Encouraged Set, Deposit Takers, Capital to Assets, Percent	    
  5. Financial Soundness Indicators, Encouraged Set, Deposit Takers, Foreign-Currency-Denominated Liabilities to Total Liabilities, Percent

---
# Scoring 

Scoring for each of five indicators:    

  * 1 point. Value available for most recent quarter   
  * 0.75 points. Value available for most recent year, but not most recent quarter    
  * 0.5 points. Value available for *next* most recent year   
  * 0.25 points. Value available in any year
  * 0 points. No values available    
  
  
Hypothetical Data Products score is average across five FSI indicators.

```{r fsi, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
...
D5.2.9.FSI.IMF <-   read_excel(path=paste(raw_dir, "5.2_DISM","Compiled Data for World Bank SPI.xlsx", sep="/" ),
                               sheet ="FSI Quarterly Timeliness",
                               skip=1
                               ) %>%
  pivot_longer(
    cols=c(
      ...
      "Financial Soundness Indicators, Core Set, Deposit Takers, Earnings and Profitability, Return on Equity, Percent",
      "Financial Soundness Indicators, Encouraged Set, Deposit Takers, Capital to Assets, Percent",
      "Financial Soundness Indicators, Encouraged Set, Deposit Takers, Foreign-Currency-Denominated Liabilities to Total Liabilities, Percent"
    ),
    names_to='FSI',
    values_to='FSI_value'
  ) %>%
  mutate(
         FSI_quarter=lubridate::quarter(FSI_value, with_year = TRUE),
         FSI_year=lubridate::year(FSI_value)) %>%
  ...
  mutate(SPI.D5.2.9.FSI.IMF=case_when(
    FSI_quarter==2020.2 ~ 1,
    FSI_year==2020 ~ 0.75,
    FSI_year==2019 ~ 0.5,
    FSI_year<2019 ~ 0.25,
    TRUE ~ 0 
  ),
  RAW.D5.2.9.FSI.IMF=FSI_value) %>%
  mutate(IMF_code=as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  select(iso3c, FSI, RAW.D5.2.9.FSI.IMF,SPI.D5.2.9.FSI.IMF ) %>%
  mutate(date=2020) 
  
D5.2.9.FSI.IMF_annual <-   read_excel(path=paste(raw_dir, "5.2_DISM","Compiled Data for World Bank SPI.xlsx", sep="/" ),
                               sheet ="FSI Annual Timeliness",
                               skip=1
                               ) %>%
  pivot_longer(
    cols=c(
  ...
  mutate(SPI.D5.2.9.FSI.IMF_annual=case_when(
    FSI_value==2020 ~ 0.75,
    FSI_value==2019 ~ 0.5,
    FSI_value<2019 ~ 0.25,
    TRUE ~ 0 
  ...
  mutate(IMF_code=as.numeric(IMF_code)) %>%
  left_join(imf_codes) %>%
  select(iso3c,FSI ,SPI.D5.2.9.FSI.IMF_annual ) %>%
  mutate(date=2020)

  
D5.2.9.FSI <-  D5.2.9.FSI.IMF %>%
  left_join(D5.2.9.FSI.IMF_annual) %>%
  rowwise() %>%
  mutate(SPI.D5.2.9.FSI=100*max(c_across(c('SPI.D5.2.9.FSI.IMF','SPI.D5.2.9.FSI.IMF_annual')) )) %>% #fill in with IMF data if missing.  This applies mostly to high income countries
  group_by(iso3c, date) %>%
  summarise(SPI.D5.2.9.FSI=mean(SPI.D5.2.9.FSI, na.rm=T)) %>%
  arrange(date, iso3c)
...
.center2[
```{r maps_5.2.9.fsi, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}

D5.2.9.FSI <- D5.2.9.FSI %>%
  left_join(wbstats::wb_countries()) %>%
  mutate(weights=1)

spi_mapper('D5.2.9.FSI', 'SPI.D5.2.9.FSI', 'IMF-SPI Dimension 3: Data Products Indicator based on Financial Soundness Indicators')
...
.center3[
```{r fsi_charts, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
spi_charts('D5.2.9.FSI', 'SPI.D5.2.9.FSI', 'IMF-SPI Dimension 3: Data Products Indicator based on Financial Soundness Indicators')
...
---
.center3[
```{r country_fsi_charts, echo=FALSE, dpi=250, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
spi_country_charts('D5.2.9.FSI', 'SPI.D5.2.9.FSI', 'IMF-SPI Dimension 3: Data Products Indicator based on Financial Soundness Indicators')
